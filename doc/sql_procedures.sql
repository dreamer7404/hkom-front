...
-- 함수 hkomms.CHECK_VALID_DATA 구조 내보내기
DELIMITER //
CREATE FUNCTION `CHECK_VALID_DATA`(P_PRDN_ORD_NO          VARCHAR(15),
										P_DEST_NAT_CD VARCHAR(5)) RETURNS varchar(1) CHARSET utf8mb3 COLLATE utf8mb3_general_ci
BEGIN
/***************************************************************************
 * Function 명칭 : CHECK_VALID_DATA
 * Function 설명 : 용도차가 아닌지의 여부를 확인하는 함수
 * 입력 파라미터    : P_PRDN_ORD_NO    생산주문번호
 *                P_DEST_NAT_CD    목적지국가코드
 * 리턴값         : V_CURR_YER       용도차가 아니면 N,맞으면 Y
 ****************************************************************************
 * 작업내용
...
-- 이벤트 hkomms.EV_CREATE_WK_DATE 구조 내보내기
DELIMITER //
CREATE EVENT `EV_CREATE_WK_DATE` ON SCHEDULE EVERY 1 YEAR STARTS '2023-01-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_CREATE_WK_DATE()//
DELIMITER ;
...
-- 함수 hkomms.FU_SPLIT 구조 내보내기
DELIMITER //
CREATE FUNCTION `FU_SPLIT`(P_LIST   VARCHAR(1000),
										    P_COUNT	 INT) RETURNS varchar(1000) CHARSET utf8mb3 COLLATE utf8mb3_general_ci
BEGIN
/***************************************************************************
 * Function 명칭 : FU_SPLIT
 * Function 설명 : 문자열 구분자를 행으로 분리 (Split)하여 리턴하는 함수
 * 입력 파라미터    : P_LIST    ','구분으로 리스트화할 문자열
 *                P_COUNT   행표현수
 * 리턴값         : V_LIST_TYPE  요청받은 문자열을 행으로 리턴해줌
 ****************************************************************************
 * 작업내용
 * 최초작성          2023-04-04     안상천   최초 전환함
 ****************************************************************************/
	DECLARE V_DEL        VARCHAR(1);    
	/*DECLARE V_LIST       VARCHAR(32767):= P_LIST;*/
	DECLARE V_LIST  	VARCHAR(1000);
	DECLARE V_CURR_IDX   INT;
	/*DECLARE V_LIST_TYPE  LIST_TYPE;*/
	DECLARE V_LIST_TYPE  VARCHAR(1000);
	DECLARE V_NUM        INT;
...
-- 프로시저 hkomms.LOAD_APS_ODR_INFO_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_APS_ODR_INFO_HMC`(IN CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_APS_ODR_INFO_HMC
 * Procedure 설명 : 차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다.
 *                 한번도 추가되지 않은 경우(무조건 Insert 해 준다.)
 *                 바로이전의 데이터가 변경되지 않은 경우에는 종료일 업데이트만 해준다.
 *                 데이터가 같은지 여부 확인하는 로직에 연식도 추가해 준다.
 *                 재(PK 조건이 같은 경우에도 특별하게 연식을 발부하는 경우가 있기 때문이다.(GET_MDL_MDY_CD_EXTRA 프로시저 참고))
 ...
 ****************************************************************************/
	...
	DECLARE V_EXPD_NAT_CD		VARCHAR(5);
	DECLARE CNT					INT;
	DECLARE V_PRDN_VEHL_CD			VARCHAR(4);
	DECLARE V_EXPD_REGN_CD		VARCHAR(4);
	
	DECLARE V_PM03F_PLNT_1 VARCHAR(2);
	DECLARE V_PM03F_MODL_1 VARCHAR(4);
	DECLARE V_PM03F_USEE_1 VARCHAR(1);
	DECLARE V_PM03F_PACK_1 VARCHAR(4);
	DECLARE V_PM03F_ORDR_1 VARCHAR(20);
	DECLARE V_PM03F_DIST_1 VARCHAR(5);
	DECLARE V_PM03F_REGN_1 VARCHAR(1);
	DECLARE V_PM03F_BMDL_1 VARCHAR(12);
	DECLARE V_PM03F_DEST_1 VARCHAR(5);
	DECLARE V_PM03F_ORDQ_1 BIGINT;
	DECLARE V_PM03F_PLAN_1 BIGINT;
	DECLARE V_PM03F_OCNN_1 VARCHAR(4);
	DECLARE V_PM03F_VERS_1 VARCHAR(3);
	...
	DECLARE APS_ODR_INFO_HMC CURSOR FOR
	   	                            SELECT PM03F_PLNT,
		 						 		   SUBSTR(PM03F_MODL, 1, 2) AS PM03F_MODL,
			   	                           PM03F_USEE,
			   	                           PM03F_PACK,
			   	                           CONCAT(PM03F_USEE , PM03F_PACK , PM03F_REGN , PM03F_SERL) AS PM03F_ORDR,
			   	                           PM03F_DIST,
			   	  		                   PM03F_REGN,
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
		    DECLARE V_RETURNED_SQLSTATE VARCHAR(10);
		    DECLARE V_MYSQL_ERRNO INT;
		    DECLARE V_MESSAGE_TEXT VARCHAR(4000);
	        SET ERR_CNT = 1;
			IF ERR_CNT > 0 THEN
				ROLLBACK;
			
		        GET DIAGNOSTICS CONDITION 1 V_RETURNED_SQLSTATE = RETURNED_SQLSTATE
		            , V_MYSQL_ERRNO = MYSQL_ERRNO
		            , V_MESSAGE_TEXT = MESSAGE_TEXT;
           
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_APS_ODR_INFO_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							...
							,',V_EXPD_NAT_CD:',IFNULL(V_EXPD_NAT_CD,'')
							,',CNT:',IFNULL(CONCAT(CNT),'')
							,',V_PRDN_VEHL_CD:',IFNULL(V_PRDN_VEHL_CD,'')
							,',V_EXPD_REGN_CD:',IFNULL(V_EXPD_REGN_CD,'')
							,',V_PM03F_PLNT_1:',IFNULL(V_PM03F_PLNT_1,'')
							,',V_PM03F_MODL_1:',IFNULL(V_PM03F_MODL_1,'')
							,',V_PM03F_USEE_1:',IFNULL(V_PM03F_USEE_1,'')
							,',V_PM03F_PACK_1:',IFNULL(V_PM03F_PACK_1,'')
							,',V_PM03F_ORDR_1:',IFNULL(V_PM03F_ORDR_1,'')
							,',V_PM03F_DIST_1:',IFNULL(V_PM03F_DIST_1,'')
							,',V_PM03F_REGN_1:',IFNULL(V_PM03F_REGN_1,'')
							,',V_PM03F_BMDL_1:',IFNULL(V_PM03F_BMDL_1,'')
							,',V_PM03F_DEST_1:',IFNULL(V_PM03F_DEST_1,'')
							,',V_PM03F_ORDQ_1:',IFNULL(CONCAT(V_PM03F_ORDQ_1),'')
							,',V_PM03F_PLAN_1:',IFNULL(CONCAT(V_PM03F_PLAN_1),'')
							,',V_PM03F_OCNN_1:',IFNULL(V_PM03F_OCNN_1,'')
							,',V_PM03F_VERS_1:',IFNULL(V_PM03F_VERS_1,'')
							,',V_EXCNT:',IFNULL(CONCAT(V_EXCNT),''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
			END IF;
	     END;
    ...
    SET CURR_LOC_NUM = 1;
	   		...
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'UPDATE TB_APS_ODR_INFO START');
    ...
    SET CURR_LOC_NUM = 3;
	   		...
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'UPDATE TB_APS_ODR_INFO END');

    SET CURR_LOC_NUM = 4;
			
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'DELETE TB_APS_ODR_INFO START');
			

    SET CURR_LOC_NUM = 5;

			/*현재일에 순수하게 입력된 항목은 삭제하도록 한다. */
			DELETE FROM TB_APS_ODR_INFO
    ...
    SET CURR_LOC_NUM = 6;
	   		...
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'DELETE TB_APS_ODR_INFO END');

    SET CURR_LOC_NUM = 7;

					   
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'FOR ODR_LIST IN APS_ODR_INFO_HMC LOOP START');
	   		SET CNT = 0;
	...
	OPEN APS_ODR_INFO_HMC; /* cursor 열기 */
	JOBLOOP1 : LOOP  /*루프명 : LOOP 시작*/
	FETCH APS_ODR_INFO_HMC INTO V_PM03F_PLNT_1,V_PM03F_MODL_1,V_PM03F_USEE_1,V_PM03F_PACK_1,V_PM03F_ORDR_1,V_PM03F_DIST_1,V_PM03F_REGN_1,V_PM03F_BMDL_1,V_PM03F_DEST_1,V_PM03F_ORDQ_1,V_PM03F_PLAN_1,V_PM03F_OCNN_1,V_PM03F_VERS_1;
	IF endOfRow1 THEN
	 LEAVE JOBLOOP1 ;
	END IF;

				/*차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다. */
				/************************************  V_PRDN_VEHL_CD  값 정의 필요 ******************/
				/*CALL SP_GET_MANUAL_INFO_HMC(CURR_YMD,
											P_EXPD_CO_CD,
											V_PM03F_MODL_1,
											V_PM03F_PACK_1,
											V_PM03F_DEST_1,
				...
				IF SUBSTR(V_PM03F_MODL_1, 1, 2) = 'H8' THEN
					IF SUBSTR(V_PM03F_BMDL_1, 3, 2) = 'W5' THEN	
						SET V_QLTY_VEHL_CD = 'YBC';	 
					ELSE	 
						SET V_QLTY_VEHL_CD = 'YB';	 
					END IF;
				ELSE
					/*품질차종코드 조회	 */
					SELECT MAX(B.QLTY_VEHL_CD)	 
					INTO V_QLTY_VEHL_CD	 
					FROM TB_ALTN_VEHL_MGMT B	 
					WHERE B.PRDN_VEHL_CD = V_PM03F_MODL_1	 
					AND B.PRVS_SCN_CD = 'A'
					AND (SELECT COUNT(C.QLTY_VEHL_CD)	 
                        FROM TB_VEHL_MGMT C	 
			            WHERE C.QLTY_VEHL_CD = B.QLTY_VEHL_CD	 
			            AND C.DL_EXPD_CO_CD = P_EXPD_CO_CD)>0;
				END IF;
			
			
				SET V_EXPD_NAT_CD = GET_NAT_CD(V_PM03F_DEST_1,P_EXPD_CO_CD); 
				SET V_MDL_MDY_CD := NULL;
						
				IF V_QLTY_VEHL_CD IS NOT NULL AND V_EXPD_NAT_CD IS NOT NULL THEN			   
					/*이전 방식 나중에 주석 처리 요망 */
					...
					AND A.DL_EXPD_NAT_CD = V_EXPD_NAT_CD
					AND A.QLTY_VEHL_CD = V_QLTY_VEHL_CD
					AND V_PM03F_PACK_1 >= B.DESMP1_CD
					AND V_PM03F_PACK_1 <= B.DEFMP1_CD;
			
					/* 신규 방식 현재는 주석 처리함(국가/언어코드 관리 화면에서 지역이 입력되어 있어야만 사용 가능)  */
					IF V_MDL_MDY_CD IS NULL THEN
						SELECT MAX(DL_EXPD_REGN_CD)
						INTO V_EXPD_REGN_CD
						FROM TB_NATL_MGMT
						WHERE DL_EXPD_CO_CD = P_EXPD_CO_CD
						AND DL_EXPD_NAT_CD = V_EXPD_NAT_CD;
			
						IF V_EXPD_REGN_CD IS NOT NULL THEN
							SELECT MAX(B.MDL_MDY_CD)
							INTO V_MDL_MDY_CD
							FROM TB_VEHL_MDY_MGMT B
							WHERE B.QLTY_VEHL_CD = V_QLTY_VEHL_CD
							AND B.DL_EXPD_REGN_CD = V_EXPD_REGN_CD
							AND V_PM03F_PACK_1 >= B.DESMP1_CD
							AND V_PM03F_PACK_1 <= B.DEFMP1_CD;
						END IF;
					END IF;
					...
					/*특수한 경우의 연식변경 작업 수행
					  별도 프로시져에서 자체 적용함	
					CALL SP_GET_MDL_MDY_CD_EXTRA_HMC(CURR_YMD,
						   						P_EXPD_CO_CD,
						   						V_QLTY_VEHL_CD,
												V_EXPD_NAT_CD,
												V_PM03F_PACK_1,
					...
					/*위로 프로시져를 자체 적용함 */
					IF V_QLTY_VEHL_CD = 'HD' AND V_EXPD_NAT_CD = 'D03' AND V_PM03F_PACK_1 = '0903' then 
						/*생산마스터 I/F 일 경우 */
						IF NOT P_EXPD_CO_CD = '01' AND NOT P_EXPD_CO_CD = '02' THEN
							IF CURR_YMD >= '20090415' THEN
								SET V_MDL_MDY_CD = '08';
							END IF;
						/*생산계획 및 오더 I/F 인 경우 */
						ELSE
							IF CURR_YMD >= '20090416' THEN
								SET V_MDL_MDY_CD = '08';
							END IF;
						END IF;
					END IF;
					IF V_QLTY_VEHL_CD = 'JB' AND V_PM03F_PACK_1 = '0907' AND SUBSTR(V_PM03F_BMDL_1, 3, 2) = 'S4' THEN 
						IF CURR_YMD >= '20090701' THEN
							SET V_MDL_MDY_CD = '10';
						END IF;
					END IF;
				END IF;
...
IF V_PM03F_MODL_1 = 'BU' AND V_PM03F_DEST_1 = 'B28AB' THEN 
		CALL WRITE_BATCH_EXE_LOG('APS FOR IN', SYSDATE(), 'F', CONCAT('V_QLTY_VEHL_CD : ' , V_QLTY_VEHL_CD 
		, ', V_MDL_MDY_CD : ' , V_MDL_MDY_CD  , ', V_EXPD_NAT_CD : ' , V_EXPD_NAT_CD
		, ', PM03F_ORDQ : ' , V_PM03F_ORDQ_1 ));
END IF;			
					
				SELECT MAX(APL_FNH_YMD)
				INTO V_APL_FNH_YMD
				FROM TB_APS_ODR_INFO A
				WHERE A.PRDN_ORD_NO = V_PM03F_ORDR_1
			    AND A.DYTM_PLN_NAT_CD = V_PM03F_DIST_1
			    AND A.PRDN_PLNT_CD = V_PM03F_PLNT_1
				AND A.DYTM_PLN_VEHL_CD = V_PM03F_MODL_1
			    AND A.USF_CD = V_PM03F_USEE_1
			    AND A.MO_PACK_CD = V_PM03F_PACK_1
				AND A.DL_EXPD_CO_CD = P_EXPD_CO_CD
...
IF V_PM03F_MODL_1 = 'BU' AND V_PM03F_DEST_1 = 'B28AB' THEN 
		CALL WRITE_BATCH_EXE_LOG('V_APL_FNH_YMD NULL', SYSDATE(), 'F', CONCAT('INSERT == V_QLTY_VEHL_CD : ' , V_QLTY_VEHL_CD 
		, ', V_MDL_MDY_CD : ' , V_MDL_MDY_CD  , ', V_EXPD_NAT_CD : ' , V_EXPD_NAT_CD 
		, ', PM03F_ORDQ : ' , V_PM03F_ORDQ_1 ));
END IF;
				...
				ELSE
				   ...
				   UPDATE TB_APS_ODR_INFO A
			 	   SET APL_FNH_YMD = CURR_YMD, /*적용완료일을 현재일로 해준다. */
					   ...
					   ORD_QTY = V_PM03F_ORDQ_1,	/* 170324 추가 */
					   MDFY_DTM = SYSDATE()				/* 170104 추가 */
				   WHERE A.PRDN_ORD_NO = V_PM03F_ORDR_1
			       AND A.DYTM_PLN_NAT_CD = V_PM03F_DIST_1
				   AND A.PRDN_PLNT_CD = V_PM03F_PLNT_1
...
IF V_PM03F_MODL_1 = 'BU' AND V_PM03F_DEST_1 = 'B28AB' THEN 
		CALL WRITE_BATCH_EXE_LOG('V_APL_FNH_YMD NOT NULL', SYSDATE(), 'F', CONCAT('UPDATE == V_QLTY_VEHL_CD : ' , V_QLTY_VEHL_CD 
		, ', V_MDL_MDY_CD : ' , V_MDL_MDY_CD  , ', V_EXPD_NAT_CD : ' , V_EXPD_NAT_CD 
		, ', PM03F_ORDQ : ' , V_PM03F_ORDQ_1 , ', PM03F_PLAN : ' , V_PM03F_PLAN_1 ));
END IF;	

				SET V_EXCNT = 0;
				SELECT COUNT(A.PRDN_ORD_NO)	 
				  INTO V_EXCNT	 
				  FROM TB_APS_ODR_INFO A
				   WHERE A.PRDN_ORD_NO = V_PM03F_ORDR_1
			       AND A.DYTM_PLN_NAT_CD = V_PM03F_DIST_1
...
IF V_PM03F_MODL_1 = 'BU' AND V_PM03F_DEST_1 = 'B28AB' THEN 
		CALL WRITE_BATCH_EXE_LOG('V_APL_FNH_YMD NOT NULL', SYSDATE(), 'F', CONCAT('INSERT == V_QLTY_VEHL_CD : ' , V_QLTY_VEHL_CD 
		, ', V_MDL_MDY_CD : ' , V_MDL_MDY_CD  , ', V_EXPD_NAT_CD : ' , V_EXPD_NAT_CD 
		, ', PM03F_ORDQ : ' , V_PM03F_ORDQ_1 ));
END IF; 
					  INSERT INTO TB_APS_ODR_INFO
	...
	END LOOP JOBLOOP1 ;
	CLOSE APS_ODR_INFO_HMC;
	 

    SET CURR_LOC_NUM = 9;

	CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_ODR_INFO_HMC', SYSDATE(), 'S', 'FOR ODR_LIST IN APS_ODR_INFO_HMC LOOP END');
...
-- 프로시저 hkomms.LOAD_APS_ODR_INFO_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_APS_ODR_INFO_KMC`(IN CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4))
BEGIN 
/***************************************************************************
 * Procedure 명칭 : LOAD_APS_ODR_INFO_KMC
 * Procedure 설명 : 현재일에 순수하게 입력된 항목은 삭제하도록 한다.
 *                 차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다.
 *                 지금까지 한번도 추가되지 않은 경우(무조건 Insert 해 준다.)	
 *                 바로이전의 데이터가 변경되지 않은 경우에는 종료일 업데이트만 해준다.
 *                 데이터가 같은지 여부 확인하는 로직에 연식도 추가해 준다.	
 ...
 ****************************************************************************/
	...
	DECLARE APS_ODR_INFO_KMC CURSOR FOR
		        SELECT PM03F_PLNT,	 
		 			   SUBSTR(PM03F_MODL, 1, 2) AS PM03F_MODL,	 
			   	       PM03F_USEE,	 
			   	       PM03F_PACK,	 
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_APS_ODR_INFO_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',V_APL_FNH_YMD:',IFNULL(V_APL_FNH_YMD,'')
							,',V_QLTY_VEHL_CD:',IFNULL(V_QLTY_VEHL_CD,'')
	...
	OPEN APS_ODR_INFO_KMC; /* cursor 열기 */
	JOBLOOP1 : LOOP  /*루프명 : LOOP 시작*/
	FETCH APS_ODR_INFO_KMC INTO V_PM03F_PLNT_1,V_PM03F_MODL_1,V_PM03F_USEE_1,V_PM03F_PACK_1,V_PM03F_ORDR_1,V_PM03F_DIST_1,V_PM03F_REGN_1,V_PM03F_BMDL_1,V_PM03F_DEST_1,V_PM03F_ORDQ_1,V_PM03F_PLAN_1,V_PM03F_OCNN_1,V_PM03F_VERS_1;
	IF endOfRow1 THEN
	 LEAVE JOBLOOP1 ;
	END IF;
				/*차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다.	  */
				/************************************  V_PRDN_VEHL_CD  값 정의 필요 ******************/
				/*CALL SP_GET_MANUAL_INFO_KMC(CURR_YMD,	 
											P_EXPD_CO_CD,	 
											V_PM03F_MODL_1,	 
											V_PM03F_PACK_1,	 
											V_PM03F_DEST_1,	 
				...
				IF V_QLTY_VEHL_CD IS NOT NULL AND V_EXPD_NAT_CD IS NOT NULL THEN
					...
					/*특수한 경우의 연식변경 작업 수행
					  별도 프로시져에서 자체 적용함	
					CALL SP_GET_MDL_MDY_CD_EXTRA_KMC(CURR_YMD,	 
						   						P_EXPD_CO_CD,	 
						   						V_QLTY_VEHL_CD,	 
												V_EXPD_NAT_CD,	 
												V_PM03F_PACK_1,	 
   				   ...
   				   UPDATE TB_APS_ODR_INFO A	 
   				    ...
   				    WHERE A.PRDN_ORD_NO = V_PM03F_ORDR_1	 
						...
						/** 배치 재실행 시 배치 오류날 경우 주석처리하고 실행 */	 
   				      /*AND A.ORD_QTY = V_PM03F_ORDQ_1	 
   			          AND A.PRDN_PLN_QTY = V_PM03F_PLAN_1	 
				      AND IFNULL(A.MDL_MDY_CD, 'NULL') = IFNULL(V_MDL_MDY_CD, 'NULL');	*/
...
-- 프로시저 hkomms.LOAD_APS_PROD_INFO_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_APS_PROD_INFO_HMC`(IN CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_APS_PROD_INFO_HMC
 * Procedure 설명 : 2주후 날짜와 현재월의 마지막날짜중에 큰값을 사용한다.
 *                 현재일에 순수하게 입력된 항목은 삭제하도록 한다.
 *                 차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다.
 *                 지금까지 한번도 추가되지 않은 경우(무조건 Insert 해 준다.)
 *                 바로이전의 데이터가 변경되지 않은 경우에는 종료일 업데이트만 해준다.
 ...
 ****************************************************************************/
	DECLARE V_APL_FNH_YMD			VARCHAR(8);
	DECLARE V_LAST_YMD				VARCHAR(8);
	DECLARE V_2WEK_YMD				VARCHAR(8);
	...
	DECLARE V_PS06A_PLGU_1 VARCHAR(1);
	DECLARE V_PS06A_BASE_1 VARCHAR(1);
	DECLARE V_PS06A_PLNT_1 VARCHAR(2);
	DECLARE V_PS06A_MODL_1 VARCHAR(2);
	DECLARE V_PS06A_EDAT_1 VARCHAR(8);
	DECLARE V_PS06A_ORDR_1 VARCHAR(9);
	DECLARE V_PS06A_DIST_1 VARCHAR(5);
	DECLARE V_PS06A_PQTY_1 BIGINT;
	...
	DECLARE APS_PROD_INFO_HMC CURSOR FOR
									SELECT T.PS06A_PLGU
									      ,T.PS06A_BASE
									      ,T.PS06A_PLNT
									      ,T.PS06A_MODL
									      ...
									      ,T.PS06A_ORDR
									      ,T.PS06A_DIST
									      ,SUM(T.PS06A_PQTY) AS PS06A_PQTY
									      ,T.DCSN_YN
									      ,T.DCSN_YMD
									      ,T.PS06A_DEST AS  PM03F_DEST
									      ,SUBSTR(T.PS06A_ORDR,2,4) AS PM03F_PACK
									      ,SUBSTR(T.PS06A_ORDR,6,1) AS PM03F_REGN
									      ,T.PS06A_BMDL AS PM03F_BMDL
									FROM   (SELECT   A.PLGU PS06A_PLGU
									                ,A.BASE PS06A_BASE
									                ,A.PLNT PS06A_PLNT
									                ,A.MODL PS06A_MODL
									                ,A.EDAT PS06A_EDAT
									                ...
									                ,A.EXCO PS06A_EXTC
									                ,A.INCO PS06A_INTC
									                ,CASE WHEN SUBSTR(ORDR,1,1) ='D' THEN 'A99VA' ELSE DEST END AS PS06A_DEST
									                ,A.PDAT PS06A_PDAT
									                ,A.BMDL PS06A_BMDL
									                ,CASE WHEN (A.PDAT <= B.PS07H_CRFD) THEN 'Y' ELSE 'N' END AS DCSN_YN
									                ,B.PS07H_CRFD AS DCSN_YMD
									                ,A.PQTY AS PS06A_PQTY
									        FROM     PSE07AB_06AC_HMC A, PSE07HB_HMC B
									        WHERE    A.PLNT = B.PS07H_PLNT
									        AND      A.PLGU = 'A'
									        AND      A.BASE = 'S' /* Sign Off 예정계획 */
									        AND      SUBSTR (CONCAT(A.EDAT), 1, 8) BETWEEN DATE_FORMAT(SYSDATE(), '%Y%m%d') AND DATE_FORMAT(DATE_ADD(SYSDATE(),INTERVAL 14 DAY), '%Y%m%d')
									        AND      A.DEST NOT LIKE '%X_'
									        AND      SUBSTR(A.ORDR,6,1) <> 'X'
									        ) T
									    GROUP BY T.PS06A_PLGU
									            ,T.PS06A_BASE
									            ,T.PS06A_PLNT
									            ,T.PS06A_MODL
									            ,T.PS06A_EDAT
									            ...
									            ,DCSN_YMD
									            ,PS06A_DEST
									            ,SUBSTR(PS06A_ORDR,2,4)
									            ,SUBSTR(PS06A_ORDR,6,1)
									            ,PS06A_BMDL; 
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_APS_PROD_INFO_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',V_APL_FNH_YMD:',IFNULL(V_APL_FNH_YMD,'')
							,',V_LAST_YMD:',IFNULL(V_LAST_YMD,'')
							,',V_2WEK_YMD:',IFNULL(V_2WEK_YMD,'')
							...
							,',V_PS06A_PLGU_1:',IFNULL(V_PS06A_PLGU_1,'')
							,',V_PS06A_BASE_1:',IFNULL(V_PS06A_BASE_1,'')
							,',V_PS06A_PLNT_1:',IFNULL(V_PS06A_PLNT_1,'')
							,',V_PS06A_MODL_1:',IFNULL(V_PS06A_MODL_1,'')
							,',V_PS06A_EDAT_1:',IFNULL(V_PS06A_EDAT_1,'')
							,',V_PS06A_ORDR_1:',IFNULL(V_PS06A_ORDR_1,'')
							,',V_PS06A_DIST_1:',IFNULL(V_PS06A_DIST_1,'')
							,',V_PS06A_PQTY_1:',IFNULL(CONCAT(V_PS06A_PQTY_1),'')
	...
	SET V_LAST_YMD = DATE_FORMAT(LAST_DAY(STR_TO_DATE(CURR_YMD, '%Y%m%d')), '%Y%m%d');
	SET V_2WEK_YMD = DATE_FORMAT(DATE_ADD(STR_TO_DATE(CURR_YMD, '%Y%m%d'), INTERVAL 14 DAY), '%Y%m%d');
	SET V_TO_YMD   = CASE WHEN V_LAST_YMD > V_2WEK_YMD THEN V_LAST_YMD ELSE V_2WEK_YMD END;

	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'UPDATE TB_APS_PROD_PLAN_INFO START');
    ...
    SET CURR_LOC_NUM = 3;
	   		...
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'UPDATE TB_APS_PROD_PLAN_INFO END');


    SET CURR_LOC_NUM = 4;

	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'DELETE TB_APS_PROD_PLAN_INFO START');

    SET CURR_LOC_NUM = 5;

			/*현재일에 순수하게 입력된 항목은 삭제하도록 한다. */
			DELETE FROM TB_APS_PROD_PLAN_INFO
			WHERE APL_STRT_YMD = CURR_YMD
			AND APL_FNH_YMD >= CURR_YMD
			AND  (SELECT COUNT(C.QLTY_VEHL_CD)	 
                        FROM TB_VEHL_MGMT C	 
    ...
    SET CURR_LOC_NUM = 6;
	   		...
	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'DELETE TB_APS_PROD_PLAN_INFO END');


    SET CURR_LOC_NUM = 7;

	   		CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'FOR PROD_LIST IN APS_PROD_INFO_HMC LOOP START');


    SET CURR_LOC_NUM = 8;

	OPEN APS_PROD_INFO_HMC; /* cursor 열기 */
	JOBLOOP1 : LOOP  /*루프명 : LOOP 시작*/
    SET CURR_LOC_NUM = 9;
	FETCH APS_PROD_INFO_HMC INTO V_PS06A_PLGU_1,V_PS06A_BASE_1,V_PS06A_PLNT_1,V_PS06A_MODL_1,V_PS06A_EDAT_1,V_PS06A_ORDR_1,V_PS06A_DIST_1,V_PS06A_PQTY_1,V_DCSN_YN_1,V_DCSN_YMD_1,V_PM03F_DEST_1,V_PM03F_PACK_1,V_PM03F_REGN_1,V_PM03F_BMDL_1;
    SET CURR_LOC_NUM = 10;
	IF endOfRow1 THEN
	 LEAVE JOBLOOP1 ;
	END IF;
    ...
    SET CURR_LOC_NUM = 11;
				/*차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다. */
				/************************************  V_PRDN_VEHL_CD  값 정의 필요 ******************/
				SET V_PRDN_VEHL_CD = V_QLTY_VEHL_CD;
				/*CALL SP_GET_MANUAL_INFO_HMC(CURR_YMD,
											P_EXPD_CO_CD,
											V_PS06A_MODL_1,
											V_PM03F_PACK_1,
				...
				/*품질차종코드 조회 */
    SET CURR_LOC_NUM = 12;
				IF SUBSTR(V_PS06A_MODL_1, 1, 2) = 'H8' THEN
    SET CURR_LOC_NUM = 13;
					IF SUBSTR(V_PM03F_BMDL_1, 3, 2) = 'W5' THEN	 
    SET CURR_LOC_NUM = 14;
						SET V_QLTY_VEHL_CD = 'YBC';	 
    ...
    SET CURR_LOC_NUM = 18;
					...
					WHERE PRDN_VEHL_CD = V_PS06A_MODL_1	 
					AND PRVS_SCN_CD = 'A';
    SET CURR_LOC_NUM = 19;
				END IF;	 
    ...
    SET CURR_LOC_NUM = 23;
					...
					/*IF V_MDL_MDY_CD IS NULL THEN
						...
						IF V_EXPD_REGN_CD IS NOT NULL THEN
							...
							AND V_PM03F_PACK_1 >= DESMP1_CD
							AND V_PM03F_PACK_1 <= DEFMP1_CD;
						END IF;
					END IF;*/
    ...
    SET CURR_LOC_NUM = 35;				
				...
				WHERE A.PRDN_ORD_NO = V_PS06A_ORDR_1
				AND A.DYTM_PLN_NAT_CD = V_PS06A_DIST_1
				AND A.PRDN_PLN_SCN_CD = V_PS06A_PLGU_1
				AND A.APL_CRTN_CD = V_PS06A_BASE_1
				AND A.PRDN_PLNT_CD = V_PS06A_PLNT_1
			    AND A.DYTM_PLN_VEHL_CD = V_PS06A_MODL_1
				AND A.PLN_PARR_YMD = V_PS06A_EDAT_1
				AND A.DL_EXPD_CO_CD = P_EXPD_CO_CD
			    AND A.APL_STRT_YMD <= CURR_YMD;
    ...
    SET CURR_LOC_NUM = 39;
				   /*바로이전의 데이터가 변경되지 않은 경우에는 종료일 업데이트만 해준다.
				     [2009-04-16]변경. 데이터가 같은지 여부 확인하는 로직에 연식도 추가해 준다. 
				     (PK 조건이 같은 경우에도 특별하게 연식을 발부하는 경우가 있기 때문이다.(GET_MDL_MDY_CD_EXTRA 프로시저 참고))   
				      MDL_MDY_CD update 하도록 수정 17.11.28 */
				   UPDATE TB_APS_PROD_PLAN_INFO A
			 	   SET APL_FNH_YMD = CURR_YMD, /*적용완료일을 현재일로 해준다. */
				       QLTY_VEHL_CD = V_QLTY_VEHL_CD,
					   MDL_MDY_CD = V_MDL_MDY_CD,
				   ...
				   /*AND A.PRDN_PLN_QTY = V_PS06A_PQTY_1
	...
	END LOOP JOBLOOP1 ;
	CLOSE APS_PROD_INFO_HMC;
	 

    SET CURR_LOC_NUM = 44;

	CALL WRITE_BATCH_EXE_LOG('APS : LOAD_APS_PROD_INFO_HMC', SYSDATE(), 'S', 'FOR PROD_LIST IN APS_PROD_INFO_HMC LOOP END');
...
-- 프로시저 hkomms.LOAD_APS_PROD_INFO_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_APS_PROD_INFO_KMC`(IN CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4))
BEGIN 
/***************************************************************************
 * Procedure 명칭 : LOAD_APS_PROD_INFO_KMC
 * Procedure 설명 : 2주후 날짜와 현재월의 마지막날짜중에 큰값을 사용한다.
 *                 현재일에 순수하게 입력된 항목은 삭제하도록 한다.
 *                 차종코드, 월팩, 목적국에 해당하는 품질차종코드, 연식, 취급설명서국가코드를 얻어온다.
 *                 바로이전의 데이터가 변경되지 않은 경우에는 종료일 업데이트만 해준다.
 ...
 ****************************************************************************/
	...
	DECLARE APS_PROD_INFO_KMC CURSOR FOR
									SELECT T.PS06A_PLGU
									      ,T.PS06A_BASE
									      ,T.PS06A_PLNT
									      ,T.PS06A_MODL
									...
									FROM   (SELECT   A.PLGU PS06A_PLGU
									                ...
									                ,B.PS07H_CRFD AS DCSN_YMD
									                ,A.PQTY AS PS06A_PQTY
									        FROM     PSE07AB_06AC_KMC A, PSE07HB_KMC B
									        WHERE    A.PLNT = B.PS07H_PLNT
									        AND      A.PLGU = 'A'
									        AND      A.BASE = 'S' /* Sign Off 예정계획 */
									        AND      SUBSTR (CONCAT(A.EDAT), 1, 8) BETWEEN DATE_FORMAT(SYSDATE(), '%Y%m%d') AND DATE_FORMAT(DATE_ADD(SYSDATE(),INTERVAL 14 DAY), '%Y%m%d')
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_APS_PROD_INFO_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',V_APL_FNH_YMD:',IFNULL(V_APL_FNH_YMD,'')
							,',V_LAST_YMD:',IFNULL(V_LAST_YMD,'')
	...
	OPEN APS_PROD_INFO_KMC; /* cursor 열기 */
	JOBLOOP1 : LOOP  /*루프명 : LOOP 시작*/
	FETCH APS_PROD_INFO_KMC INTO V_PS06A_PLGU_1,V_PS06A_BASE_1,V_PS06A_PLNT_1,V_PS06A_MODL_1,V_PS06A_EDAT_1,V_PS06A_ORDR_1,V_PS06A_DIST_1,V_PS06A_PQTY_1,V_DCSN_YN_1,V_DCSN_YMD_1,V_PM03F_DEST_1,V_PM03F_PACK_1,V_PM03F_REGN_1,V_PM03F_BMDL_1;
	IF endOfRow1 THEN
	 LEAVE JOBLOOP1 ;
	END IF; 
...
-- 프로시저 hkomms.LOAD_DOM_SALE_YMD 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_DOM_SALE_YMD`(IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_DOM_SALE_YMD
 * Procedure 설명 : 내수 출고일 정보 로드 프로시저
...
-- 프로시저 hkomms.LOAD_PROD_MST_ERP_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_PROD_MST_ERP_HMC`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_APL_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4),
                                        IN P_ET_GUBN_CD VARCHAR(2))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_PROD_MST_ERP_HMC
 * Procedure 설명 : ERP로 부터 생산실적 로드
 *                 용도차가 아닌 경우에만 작업을 진행한다.
 *                 연식이 지정되지 않은 경우에는 임시로 PPMS 연식을 이용하여 정보를 조회한다.(상용 차종을 위한 조치사항)
 *                 투입일자를 현재공정위치일자와 동일하게 처리해 준다.
 *                 블랙 리스트 관련 조회 후 처리
 *                 현재 공정이 투입공정 이전의 공정이라면 아래의 작업을 수행한다.
 *                 특수한 경우의 연식변경 작업 다시 한번 수행(APL_YMD가 바뀔 경우 연식이 달라질 수 있음)
 *                 현재 공정이 투입공정 이후의 공정이라면 아래의 작업을 수행한다.
 *                 특수한 경우의 연식변경 작업 다시 한번 수행(APL_YMD가 바뀔 경우 연식이 달라질 수 있음)
 *                 아직 한번도 투입되지 않은 공정이라면 투입정보 테이블에 데이터를 추가해 준다.
 ...
 ****************************************************************************/
	...
	DECLARE V_PRDN_MST_VEHL_CD_1 VARCHAR(4);
	DECLARE V_BN_SN_1 VARCHAR(6);
	DECLARE V_APL_YMD_1 VARCHAR(8);
	DECLARE V_USF_CD_1 VARCHAR(2);
	DECLARE V_MO_PACK_CD_1 VARCHAR(4);
	DECLARE V_PRDN_ORD_NO_1 VARCHAR(15);
	DECLARE V_PRDN_MST_NAT_CD_1 VARCHAR(5);
	DECLARE V_BASC_MDL_CD_1 VARCHAR(12);
	DECLARE V_PRDN_OCN_CD_1 VARCHAR(4);
	DECLARE V_VER_CD_1 VARCHAR(3);
	DECLARE V_DEST_NAT_CD_1 VARCHAR(5);
	DECLARE V_POW_LOC_CD_1 VARCHAR(4);
	DECLARE V_TH1_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH2_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH3_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH4_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH5_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH6_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH7_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH8_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_TH9_POW_STRT_YMDHM_1 VARCHAR(12);
	DECLARE V_T10PS1_YMDHM_1 VARCHAR(12);
	DECLARE V_T11PS1_YMDHM_1 VARCHAR(12);
	DECLARE V_MDL_MDY_CD_1 VARCHAR(4);
	DECLARE V_VIN_1 VARCHAR(17);
	DECLARE V_PLNT_CD_1 VARCHAR(3);
	DECLARE V_TH1_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH2_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH3_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH4_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH5_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH6_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH7_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH8_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_TH9_POW_STRT_YMD_1 VARCHAR(8);
	DECLARE V_T10PS1_YMD_1 VARCHAR(8);
	DECLARE V_T11PS1_YMD_1 VARCHAR(8);
	
	DECLARE V_EXCNT			        INT;

	DECLARE SAM_PRDN_MST_VEHL_CD       CHAR(3);
	DECLARE SAM_BN_SN                  CHAR(6);
	DECLARE SAM_USF_CD                 CHAR(1);
	DECLARE SAM_MO_PACK_CD             CHAR(4);
	DECLARE SAM_PRDN_ORD_NO            CHAR(9);
	DECLARE SAM_PRDN_MST_NAT_CD        CHAR(5);
	DECLARE SAM_BASC_MDL_CD            CHAR(12);
	DECLARE SAM_PRDN_OCN_CD            CHAR(4);
	DECLARE SAM_VER_CD                 CHAR(3);
	DECLARE SAM_DEST_NAT_CD            CHAR(5);
	DECLARE SAM_TH1_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH2_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH3_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH4_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH5_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH6_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH7_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH8_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_TH9_POW_STRT_YMDHM     CHAR(12);
	DECLARE SAM_T10PS1_YMDHM           CHAR(12);
	DECLARE SAM_T11PS1_YMDHM           CHAR(12);
	DECLARE SAM_T12PS1_YMDHM           CHAR(12);
	DECLARE SAM_T13PS1_YMDHM           CHAR(12);
	DECLARE SAM_T14PS1_YMDHM           CHAR(12);
	DECLARE SAM_T15PS1_YMDHM           CHAR(12);
	DECLARE SAM_T16PS1_YMDHM           CHAR(12);
	DECLARE SAM_TH1_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH2_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH3_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH4_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH5_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH6_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH7_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH8_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_TH9_POW_STRT_YMD       CHAR(8);
	DECLARE SAM_T10PS1_YMD             CHAR(8);
	DECLARE SAM_T11PS1_YMD             CHAR(8);
	DECLARE SAM_T12PS1_YMD             CHAR(8);
	DECLARE SAM_T13PS1_YMD             CHAR(8);
	DECLARE SAM_T14PS1_YMD             CHAR(8);
	DECLARE SAM_T15PS1_YMD             CHAR(8);
	DECLARE SAM_T16PS1_YMD             CHAR(8);
	DECLARE SAM_VIN                    CHAR(17);

	DECLARE SAM_POW_LOC_CD             CHAR(2);     /*PPMS 공정위치코드 */
	DECLARE V_FNL_LOC_CD               VARCHAR(2); /*최종 공정위치 코드 */
	DECLARE V_TRWI_LOC_CD              VARCHAR(2); /*승용,상용,내수 구분에 따른 투입 기준 공정 위치 값을 보관하는 변수 */
		                                         /*(10, 11, 16 공정 코드 중 하나임) */

	DECLARE SAM_MDL_MDY_CD             CHAR(2);     /*PPMS 연식코드 */
	DECLARE V_MDL_MDY_CD               VARCHAR(2); /*연식코드 */

	DECLARE SAM_APL_YMD                CHAR(8);     /*현재공정위치 일자 */
	DECLARE V_TRWI_YMD                 VARCHAR(8); /*투입일자 */
	DECLARE V_TRWI_USED_YN             CHAR(1);     /*이전에 이미 투입되었는지의 여부를 보관하는 변수 */
	DECLARE V_PREV_TRWI_YMD            VARCHAR(8); /*이전에 이미 투입된 경우의 해당 투입일을 보관하는 변수 */
	DECLARE V_PREV_APL_YMD				VARCHAR(8); /*이전에 이미 투입된 경우의 해당 투입처리된 적용일을 보관하는 변수. 추가 : 임용석 - 2016.07.01 */

	DECLARE V_QLTY_VEHL_CD  			VARCHAR(4); /*품질차종코드 */
	DECLARE V_EXPD_NAT_CD   			VARCHAR(5); /*취급설명서국가코드 */
	DECLARE V_PRDN2_VEHL_CD            VARCHAR(4); /*생산차종코드 */

	DECLARE V_PAC_SCN_CD               VARCHAR(4); /*승상구분코드 */
	DECLARE V_PDI_CD		            VARCHAR(4); /*PDI구분코드 */

	DECLARE V_TMP_TRWI_YMDHM           VARCHAR(12);
	DECLARE V_TMP_TRWI_YMD			    VARCHAR(8);

	DECLARE SAM_PLNT_CD				VARCHAR(3); /*[추가]. 2010.04.13.김동근 공장코드 신규 추가 */
	DECLARE V_SALE_YMD					VARCHAR(8); /*[추가] 내수용 판매일자 보관  */
	DECLARE V_WIOUT_DESNAT             VARCHAR(1); /*[추가]. 2016.06.13.블랙리스트 제외처리 JHKIM */
	DECLARE V_EXPD_DOM_NAT_CD VARCHAR(6);
	...
	DECLARE PROD_MST_ERP_LIST CURSOR FOR
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_PROD_MST_ERP_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_APL_YMD:',IFNULL(P_APL_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
							,',V_CURR_YMD:',IFNULL(V_CURR_YMD,'')
							,',LOOP_CNT:',IFNULL(CONCAT(LOOP_CNT),'')
							,',V_EXPD_REGN_CD:',IFNULL(V_EXPD_REGN_CD,'')
							,',V_PRDN_MST_VEHL_CD_1:',IFNULL(V_PRDN_MST_VEHL_CD_1,'')
							,',V_BN_SN_1:',IFNULL(V_BN_SN_1,'')
							,',V_APL_YMD_1:',IFNULL(V_APL_YMD_1,'')
							,',V_USF_CD_1:',IFNULL(V_USF_CD_1,'')
							,',V_MO_PACK_CD_1:',IFNULL(V_MO_PACK_CD_1,'')
							,',V_PRDN_ORD_NO_1:',IFNULL(V_PRDN_ORD_NO_1,'')
							,',V_PRDN_MST_NAT_CD_1:',IFNULL(V_PRDN_MST_NAT_CD_1,'')
							,',V_BASC_MDL_CD_1:',IFNULL(V_BASC_MDL_CD_1,'')
							,',V_PRDN_OCN_CD_1:',IFNULL(V_PRDN_OCN_CD_1,'')
							,',V_VER_CD_1:',IFNULL(V_VER_CD_1,'')
							,',V_DEST_NAT_CD_1:',IFNULL(V_DEST_NAT_CD_1,'')
							,',V_POW_LOC_CD_1:',IFNULL(V_POW_LOC_CD_1,'')
							,',V_TH1_POW_STRT_YMDHM_1:',IFNULL(V_TH1_POW_STRT_YMDHM_1,'')
							,',V_TH2_POW_STRT_YMDHM_1:',IFNULL(V_TH2_POW_STRT_YMDHM_1,'')
							,',V_TH3_POW_STRT_YMDHM_1:',IFNULL(V_TH3_POW_STRT_YMDHM_1,'')
							,',V_TH4_POW_STRT_YMDHM_1:',IFNULL(V_TH4_POW_STRT_YMDHM_1,'')
							,',V_TH5_POW_STRT_YMDHM_1:',IFNULL(V_TH5_POW_STRT_YMDHM_1,'')
							,',V_TH6_POW_STRT_YMDHM_1:',IFNULL(V_TH6_POW_STRT_YMDHM_1,'')
							,',V_TH7_POW_STRT_YMDHM_1:',IFNULL(V_TH7_POW_STRT_YMDHM_1,'')
							,',V_TH8_POW_STRT_YMDHM_1:',IFNULL(V_TH8_POW_STRT_YMDHM_1,'')
							,',V_TH9_POW_STRT_YMDHM_1:',IFNULL(V_TH9_POW_STRT_YMDHM_1,'')
							,',V_T10PS1_YMDHM_1:',IFNULL(V_T10PS1_YMDHM_1,'')
							,',V_T11PS1_YMDHM_1:',IFNULL(V_T11PS1_YMDHM_1,'')
							,',V_MDL_MDY_CD_1:',IFNULL(V_MDL_MDY_CD_1,'')
							,',V_VIN_1:',IFNULL(V_VIN_1,'')
							,',V_PLNT_CD_1:',IFNULL(V_PLNT_CD_1,'')
							,',V_TH1_POW_STRT_YMD_1:',IFNULL(V_TH1_POW_STRT_YMD_1,'')
							,',V_TH2_POW_STRT_YMD_1:',IFNULL(V_TH2_POW_STRT_YMD_1,'')
							,',V_TH3_POW_STRT_YMD_1:',IFNULL(V_TH3_POW_STRT_YMD_1,'')
							,',V_TH4_POW_STRT_YMD_1:',IFNULL(V_TH4_POW_STRT_YMD_1,'')
							,',V_TH5_POW_STRT_YMD_1:',IFNULL(V_TH5_POW_STRT_YMD_1,'')
							,',V_TH6_POW_STRT_YMD_1:',IFNULL(V_TH6_POW_STRT_YMD_1,'')
							,',V_TH7_POW_STRT_YMD_1:',IFNULL(V_TH7_POW_STRT_YMD_1,'')
							,',V_TH8_POW_STRT_YMD_1:',IFNULL(V_TH8_POW_STRT_YMD_1,'')
							,',V_TH9_POW_STRT_YMD_1:',IFNULL(V_TH9_POW_STRT_YMD_1,'')
							,',V_T10PS1_YMD_1:',IFNULL(V_T10PS1_YMD_1,'')
							,',V_T11PS1_YMD_1:',IFNULL(V_T11PS1_YMD_1,'')
							,',V_EXCNT:',IFNULL(CONCAT(V_EXCNT),'')
							,',SAM_PRDN_MST_VEHL_CD:',IFNULL(SAM_PRDN_MST_VEHL_CD,'')
							,',SAM_BN_SN:',IFNULL(SAM_BN_SN,'')
							,',SAM_USF_CD:',IFNULL(SAM_USF_CD,'')
							,',SAM_MO_PACK_CD:',IFNULL(SAM_MO_PACK_CD,'')
							,',SAM_PRDN_ORD_NO:',IFNULL(SAM_PRDN_ORD_NO,'')
							,',SAM_PRDN_MST_NAT_CD:',IFNULL(SAM_PRDN_MST_NAT_CD,'')
							,',SAM_BASC_MDL_CD:',IFNULL(SAM_BASC_MDL_CD,'')
							,',SAM_PRDN_OCN_CD:',IFNULL(SAM_PRDN_OCN_CD,'')
							,',SAM_VER_CD:',IFNULL(SAM_VER_CD,'')
							,',SAM_DEST_NAT_CD:',IFNULL(SAM_DEST_NAT_CD,'')
							,',SAM_TH1_POW_STRT_YMDHM:',IFNULL(SAM_TH1_POW_STRT_YMDHM,'')
							,',SAM_TH2_POW_STRT_YMDHM:',IFNULL(SAM_TH2_POW_STRT_YMDHM,'')
							,',SAM_TH3_POW_STRT_YMDHM:',IFNULL(SAM_TH3_POW_STRT_YMDHM,'')
							,',SAM_TH4_POW_STRT_YMDHM:',IFNULL(SAM_TH4_POW_STRT_YMDHM,'')
							,',SAM_TH5_POW_STRT_YMDHM:',IFNULL(SAM_TH5_POW_STRT_YMDHM,'')
							,',SAM_TH6_POW_STRT_YMDHM:',IFNULL(SAM_TH6_POW_STRT_YMDHM,'')
							,',SAM_TH7_POW_STRT_YMDHM:',IFNULL(SAM_TH7_POW_STRT_YMDHM,'')
							,',SAM_TH8_POW_STRT_YMDHM:',IFNULL(SAM_TH8_POW_STRT_YMDHM,'')
							,',SAM_TH9_POW_STRT_YMDHM:',IFNULL(SAM_TH9_POW_STRT_YMDHM,'')
							,',SAM_T10PS1_YMDHM:',IFNULL(SAM_T10PS1_YMDHM,'')
							,',SAM_T11PS1_YMDHM:',IFNULL(SAM_T11PS1_YMDHM,'')
							,',SAM_T12PS1_YMDHM:',IFNULL(SAM_T12PS1_YMDHM,'')
							,',SAM_T13PS1_YMDHM:',IFNULL(SAM_T13PS1_YMDHM,'')
							,',SAM_T14PS1_YMDHM:',IFNULL(SAM_T14PS1_YMDHM,'')
							,',SAM_T15PS1_YMDHM:',IFNULL(SAM_T15PS1_YMDHM,'')
							,',SAM_T16PS1_YMDHM:',IFNULL(SAM_T16PS1_YMDHM,'')
							,',SAM_TH1_POW_STRT_YMD:',IFNULL(SAM_TH1_POW_STRT_YMD,'')
							,',SAM_TH2_POW_STRT_YMD:',IFNULL(SAM_TH2_POW_STRT_YMD,'')
							,',SAM_TH3_POW_STRT_YMD:',IFNULL(SAM_TH3_POW_STRT_YMD,'')
							,',SAM_TH4_POW_STRT_YMD:',IFNULL(SAM_TH4_POW_STRT_YMD,'')
							,',SAM_TH5_POW_STRT_YMD:',IFNULL(SAM_TH5_POW_STRT_YMD,'')
							,',SAM_TH6_POW_STRT_YMD:',IFNULL(SAM_TH6_POW_STRT_YMD,'')
							,',SAM_TH7_POW_STRT_YMD:',IFNULL(SAM_TH7_POW_STRT_YMD,'')
							,',SAM_TH8_POW_STRT_YMD:',IFNULL(SAM_TH8_POW_STRT_YMD,'')
							,',SAM_TH9_POW_STRT_YMD:',IFNULL(SAM_TH9_POW_STRT_YMD,'')
							,',SAM_T10PS1_YMD:',IFNULL(SAM_T10PS1_YMD,'')
							,',SAM_T11PS1_YMD:',IFNULL(SAM_T11PS1_YMD,'')
							,',SAM_T12PS1_YMD:',IFNULL(SAM_T12PS1_YMD,'')
							,',SAM_T13PS1_YMD:',IFNULL(SAM_T13PS1_YMD,'')
							,',SAM_T14PS1_YMD:',IFNULL(SAM_T14PS1_YMD,'')
							,',SAM_T15PS1_YMD:',IFNULL(SAM_T15PS1_YMD,'')
							,',SAM_T16PS1_YMD:',IFNULL(SAM_T16PS1_YMD,'')
							,',SAM_VIN:',IFNULL(SAM_VIN,'')
							,',SAM_POW_LOC_CD:',IFNULL(SAM_POW_LOC_CD,'')
							,',V_FNL_LOC_CD:',IFNULL(V_FNL_LOC_CD,'')
							,',V_TRWI_LOC_CD:',IFNULL(V_TRWI_LOC_CD,'')
							,',SAM_MDL_MDY_CD:',IFNULL(SAM_MDL_MDY_CD,'')
							,',V_MDL_MDY_CD:',IFNULL(V_MDL_MDY_CD,'')
							,',SAM_APL_YMD:',IFNULL(SAM_APL_YMD,'')
							,',V_TRWI_YMD:',IFNULL(V_TRWI_YMD,'')
							,',V_TRWI_USED_YN:',IFNULL(V_TRWI_USED_YN,'')
							,',V_PREV_TRWI_YMD:',IFNULL(V_PREV_TRWI_YMD,'')
							,',V_PREV_APL_YMD:',IFNULL(V_PREV_APL_YMD,'')
							,',V_QLTY_VEHL_CD:',IFNULL(V_QLTY_VEHL_CD,'')
							,',V_EXPD_NAT_CD:',IFNULL(V_EXPD_NAT_CD,'')
							,',V_PRDN2_VEHL_CD:',IFNULL(V_PRDN2_VEHL_CD,'')
							,',V_PAC_SCN_CD:',IFNULL(V_PAC_SCN_CD,'')
							,',V_PDI_CD:',IFNULL(V_PDI_CD,'')
							,',V_TMP_TRWI_YMDHM:',IFNULL(V_TMP_TRWI_YMDHM,'')
							,',V_TMP_TRWI_YMD:',IFNULL(V_TMP_TRWI_YMD,'')
							,',SAM_PLNT_CD:',IFNULL(SAM_PLNT_CD,'')
							,',V_SALE_YMD:',IFNULL(V_SALE_YMD,'')
							,',V_WIOUT_DESNAT:',IFNULL(V_WIOUT_DESNAT,''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
			END IF;
	...
	SET LOOP_CNT = 0;
    SET V_EXPD_DOM_NAT_CD = 'A99VA';
    SET V_DATE_DIFF_CNT = 7;
	   		
	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ':LOAD_PROD_MST_ERP_HMC > CURR_YMD:' , P_CURR_YMD , ':APL_YMD:' , P_APL_YMD , ':ET_GUBN_CD:' , P_ET_GUBN_CD , ':'));


    SET CURR_LOC_NUM = 2;

	OPEN PROD_MST_ERP_LIST; /* cursor 열기 */
	JOBLOOP1 : LOOP  /*루프명 : LOOP 시작*/
	FETCH PROD_MST_ERP_LIST INTO V_PRDN_MST_VEHL_CD_1,V_BN_SN_1,V_APL_YMD_1,V_USF_CD_1,V_MO_PACK_CD_1,V_PRDN_ORD_NO_1,V_PRDN_MST_NAT_CD_1,V_BASC_MDL_CD_1,V_PRDN_OCN_CD_1,V_VER_CD_1,V_DEST_NAT_CD_1,V_POW_LOC_CD_1,V_TH1_POW_STRT_YMDHM_1,V_TH2_POW_STRT_YMDHM_1,V_TH3_POW_STRT_YMDHM_1,V_TH4_POW_STRT_YMDHM_1,V_TH5_POW_STRT_YMDHM_1,V_TH6_POW_STRT_YMDHM_1,V_TH7_POW_STRT_YMDHM_1,V_TH8_POW_STRT_YMDHM_1,V_TH9_POW_STRT_YMDHM_1,V_T10PS1_YMDHM_1 ,V_T11PS1_YMDHM_1,V_MDL_MDY_CD_1,V_VIN_1,V_PLNT_CD_1,V_TH1_POW_STRT_YMD_1,V_TH2_POW_STRT_YMD_1,V_TH3_POW_STRT_YMD_1,V_TH4_POW_STRT_YMD_1,V_TH5_POW_STRT_YMD_1,V_TH6_POW_STRT_YMD_1,V_TH7_POW_STRT_YMD_1,V_TH8_POW_STRT_YMD_1,V_TH9_POW_STRT_YMD_1,V_T10PS1_YMD_1,V_T11PS1_YMD_1;
	IF endOfRow1 THEN
	 LEAVE JOBLOOP1 ;
	END IF;
		       ...
		       * DATA PARSING *
		       ***************/
			   SET SAM_PRDN_MST_VEHL_CD   = V_PRDN_MST_VEHL_CD_1;
               SET SAM_BN_SN              = V_BN_SN_1;
               SET SAM_USF_CD             = V_USF_CD_1;
               SET SAM_MO_PACK_CD         = V_MO_PACK_CD_1;
               SET SAM_PRDN_ORD_NO        = V_PRDN_ORD_NO_1;
               SET SAM_PRDN_MST_NAT_CD    = V_PRDN_MST_NAT_CD_1;
               SET SAM_BASC_MDL_CD        = V_BASC_MDL_CD_1;
               SET SAM_PRDN_OCN_CD        = V_PRDN_OCN_CD_1;
               SET SAM_VER_CD             = V_VER_CD_1;

			   IF SAM_USF_CD = 'D' THEN
			   	   SET SAM_DEST_NAT_CD = V_EXPD_DOM_NAT_CD;  /*내수인 경우에는 국가코드를 A99VA로 입력하도록 한다. */
			   ELSE
			   	   SET SAM_DEST_NAT_CD = V_DEST_NAT_CD_1;
			   END IF;

               SET SAM_TH1_POW_STRT_YMDHM = IFNULL(V_TH1_POW_STRT_YMDHM_1, '000000000000');
               IF SAM_TH1_POW_STRT_YMDHM IS NULL OR SAM_TH1_POW_STRT_YMDHM = '000000000000' THEN
					 SET SAM_TH1_POW_STRT_YMDHM = CONCAT(P_APL_YMD , '0000');
               END IF;

               SET SAM_TH2_POW_STRT_YMDHM = IFNULL(V_TH2_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH3_POW_STRT_YMDHM = IFNULL(V_TH2_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH4_POW_STRT_YMDHM = IFNULL(V_TH3_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH5_POW_STRT_YMDHM = IFNULL(V_TH4_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH6_POW_STRT_YMDHM = IFNULL(V_TH5_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH7_POW_STRT_YMDHM = IFNULL(V_TH6_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH8_POW_STRT_YMDHM = IFNULL(V_TH7_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_TH9_POW_STRT_YMDHM = IFNULL(V_TH8_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_T10PS1_YMDHM       = IFNULL(V_TH9_POW_STRT_YMDHM_1, '000000000000');
               SET SAM_T11PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');
               SET SAM_T12PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');
               SET SAM_T13PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');
               SET SAM_T14PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');
               SET SAM_T15PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');
               SET SAM_T16PS1_YMDHM       = IFNULL(V_T10PS1_YMDHM_1,       '000000000000');

			   SET SAM_TH1_POW_STRT_YMD   = IFNULL(V_TH1_POW_STRT_YMD_1, '00000000');
               SET SAM_TH2_POW_STRT_YMD   = IFNULL(V_TH2_POW_STRT_YMD_1, '00000000');
               SET SAM_TH3_POW_STRT_YMD   = IFNULL(V_TH2_POW_STRT_YMD_1, '00000000');
               SET SAM_TH4_POW_STRT_YMD   = IFNULL(V_TH3_POW_STRT_YMD_1, '00000000');
               SET SAM_TH5_POW_STRT_YMD   = IFNULL(V_TH4_POW_STRT_YMD_1, '00000000');
               SET SAM_TH6_POW_STRT_YMD   = IFNULL(V_TH5_POW_STRT_YMD_1, '00000000');
               SET SAM_TH7_POW_STRT_YMD   = IFNULL(V_TH6_POW_STRT_YMD_1, '00000000');
               SET SAM_TH8_POW_STRT_YMD   = IFNULL(V_TH7_POW_STRT_YMD_1, '00000000');
               SET SAM_TH9_POW_STRT_YMD   = IFNULL(V_TH8_POW_STRT_YMD_1, '00000000');
               SET SAM_T10PS1_YMD         = IFNULL(V_TH9_POW_STRT_YMD_1, '00000000');
               SET SAM_T11PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');
               SET SAM_T12PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');
               SET SAM_T13PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');
               SET SAM_T14PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');
               SET SAM_T15PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');
               SET SAM_T16PS1_YMD         = IFNULL(V_T10PS1_YMD_1, '00000000');

			   SET SAM_MDL_MDY_CD         = GET_MDY_CD(P_EXPD_CO_CD,TRIM(V_MDL_MDY_CD_1));        /*PPMS 연식 코드 */
               SET SAM_VIN                = V_VIN_1;
			   SET SAM_PLNT_CD            = GET_PRDN_PLNT_CD_ERP(V_PLNT_CD_1, P_EXPD_CO_CD); /*[추가]. 2010.04.13.김동근 공장코드 신규 추가	  */
               SET SAM_POW_LOC_CD         = GET_POW_LOC_CD_ERP(V_POW_LOC_CD_1, P_EXPD_CO_CD); /*PPMS 공정위치 코드 */

			   /*[추가- 2009-02-09] 00공정 데이터 역시 읽어들이지 않는다.(현재 필요없어짐)
			     용도차가 아닌 경우에만 작업을 진행한다. */
			   IF SAM_POW_LOC_CD <> '00' AND CHECK_VALID_DATA(SAM_PRDN_ORD_NO, SAM_DEST_NAT_CD) = 'Y' THEN  
				   /*00공정의 경우 현공정위치의 날짜가 변경일시가 된다. */	 
				   IF SAM_POW_LOC_CD = '01' THEN
    			   	  SET SAM_APL_YMD = SAM_TH1_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '02' THEN
				   	  SET SAM_APL_YMD = SAM_TH2_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '03' THEN
    			      SET SAM_APL_YMD = SAM_TH3_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '04' THEN
    			      SET SAM_APL_YMD = SAM_TH4_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '05' THEN
    			      SET SAM_APL_YMD = SAM_TH5_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '06' THEN
    			      SET SAM_APL_YMD = SAM_TH6_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '07' THEN
    			      SET SAM_APL_YMD = SAM_TH7_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '08' THEN
    			      SET SAM_APL_YMD = SAM_TH8_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '09' THEN
    			      SET SAM_APL_YMD = SAM_TH9_POW_STRT_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '10' THEN
    			      SET SAM_APL_YMD = SAM_T10PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '11' THEN
    			      SET SAM_APL_YMD = SAM_T11PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '12' THEN
    			      SET SAM_APL_YMD = SAM_T12PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '13' THEN
    			      SET SAM_APL_YMD = SAM_T13PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '14' THEN
    			      SET SAM_APL_YMD = SAM_T14PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '15' THEN
    			      SET SAM_APL_YMD = SAM_T15PS1_YMD;
    			   ELSEIF SAM_POW_LOC_CD = '16' THEN
    			      SET SAM_APL_YMD = SAM_T16PS1_YMD;
    			   END IF;

				   /*CALL SP_GET_MANUAL_INFO_HMC(SAM_APL_YMD,	
				   				   P_EXPD_CO_CD,
				...
				/*상단 프로시져 적용 */
				IF SUBSTR(SAM_PRDN_MST_VEHL_CD, 1, 2) = 'H8' THEN
					IF SUBSTR(SAM_BASC_MDL_CD, 3, 2) = 'W5' THEN	 
						SET V_QLTY_VEHL_CD = 'YBC';	 
					ELSE	 
						SET V_QLTY_VEHL_CD = 'YB';	 
					END IF;
				ELSE
					/*품질차종코드 조회	 */
					SELECT MAX(A.QLTY_VEHL_CD)	 
					INTO V_QLTY_VEHL_CD	 
					FROM TB_ALTN_VEHL_MGMT	A 
					WHERE A.PRDN_VEHL_CD = SAM_PRDN_MST_VEHL_CD	 
					AND A.PRVS_SCN_CD = 'A';
				
				
				END IF;	 
			
				SET V_EXPD_NAT_CD = GET_NAT_CD(SAM_DEST_NAT_CD,P_EXPD_CO_CD); 
				SET V_MDL_MDY_CD = NULL;
				...
				IF V_QLTY_VEHL_CD IS NOT NULL AND V_EXPD_NAT_CD IS NOT NULL THEN			   
					...
					AND A.DL_EXPD_NAT_CD = V_EXPD_NAT_CD
					AND A.QLTY_VEHL_CD = V_QLTY_VEHL_CD
					AND SAM_MO_PACK_CD >= B.DESMP1_CD
					AND SAM_MO_PACK_CD <= B.DEFMP1_CD;
					...
					/*IF V_MDL_MDY_CD IS NULL THEN
						...
						IF V_EXPD_REGN_CD IS NOT NULL THEN
							...
							WHERE QLTY_VEHL_CD = V_QLTY_VEHL_CD
							AND DL_EXPD_REGN_CD = V_EXPD_REGN_CD
							AND SAM_MO_PACK_CD >= DESMP1_CD
							AND SAM_MO_PACK_CD <= DEFMP1_CD;
						END IF;
					END IF;*/
			
					/*특수한 경우의 연식변경 작업 수행
					  별도 프로시져에서 자체 적용함	
					CALL SP_GET_MDL_MDY_CD_EXTRA_HMC(SAM_APL_YMD,
						   						P_EXPD_CO_CD,
						   						V_QLTY_VEHL_CD,
												V_EXPD_NAT_CD,
												SAM_MO_PACK_CD,
					...
					/*위로 프로시져를 자체 적용함 */
					IF V_QLTY_VEHL_CD = 'JB' AND SAM_MO_PACK_CD = '0907' AND SUBSTR(SAM_BASC_MDL_CD, 3, 2) = 'S4' THEN 
						IF SAM_APL_YMD >= '20090701' THEN
							SET V_MDL_MDY_CD = '10';
						END IF;
					END IF;
					IF V_QLTY_VEHL_CD = 'HD' AND V_EXPD_NAT_CD = 'D03' AND SAM_MO_PACK_CD = '0903' THEN
						/*생산마스터 I/F 일 경우 */
						IF P_EXPD_CO_CD = '03' THEN
							IF SAM_APL_YMD >= '20090415' THEN
								SET V_MDL_MDY_CD = '08';
							END IF;
						/*생산계획 및 오더 I/F 인 경우 */
						ELSE
							IF SAM_APL_YMD >= '20090416' THEN
								SET V_MDL_MDY_CD = '08';
							END IF;
						END IF;
					END IF;
				...
				/*상단 프로시져 적용 종료 */
				   ...
				   IF V_MDL_MDY_CD IS NOT NULL THEN
					   SELECT MAX(A.DL_EXPD_PAC_SCN_CD),
					          MAX(A.DL_EXPD_PDI_CD)
				       INTO V_PAC_SCN_CD,
		 				    V_PDI_CD
				       FROM TB_VEHL_MGMT A
				       WHERE A.QLTY_VEHL_CD = V_QLTY_VEHL_CD
				       AND A.MDL_MDY_CD = V_MDL_MDY_CD;
				   ELSE
				   ...
				   END IF;
					...
					IF ( V_PAC_SCN_CD IS NULL  OR V_PDI_CD IS NULL ) THEN /* AND SAM_DEST_NAT_CD <> 'A99VA'  */
								CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'F', CONCAT('TB_VEHL_MGMT에 해당 차종,연식 없음!!!   PRDN_MST_VEHL_CD : ' , SAM_PRDN_MST_VEHL_CD , 
				   ...
				   SET V_TRWI_YMD    = SAM_APL_YMD;
				   SET V_FNL_LOC_CD  = SAM_POW_LOC_CD;
				   SET V_TRWI_LOC_CD = NULL;
				   SET V_TMP_TRWI_YMD = NULL;	/* 추가. 임용석 2016.07.01 */
					  ...
					  IF V_PAC_SCN_CD = '01' THEN
						 ...
						 IF V_PDI_CD = '04' THEN
							IF SAM_POW_LOC_CD >= '11' THEN
			  				    SET V_TRWI_LOC_CD  = '11';
							    SET V_TMP_TRWI_YMD = SAM_T11PS1_YMD;
							END IF;
						 ELSE
							/*그외 PDI 승용의 경우에는 10공정 부터 투입된 것으로 본다. */
						    IF SAM_POW_LOC_CD >= '10' THEN
                                SET V_TRWI_LOC_CD = '10';
								SET V_TMP_TRWI_YMD = SAM_T10PS1_YMD;
               ...
               IF V_WIOUT_DESNAT = 'Y' THEN   /* 제외 대리점 코드가 아니면 실행 */
				   ...
				   ELSE
					   ...
					   /*아직 한번도 투입되지 않은 공정이라면...
					     투입정보 테이블에 데이터를 추가해 준다. 현대만 적용,기아는 미적용*/
                   ...
                   /*[점검]
				    현재 진행중인 물량 정보 업데이트 작업 수행 */
				   CALL SP_UPDATE_PROD_MST_PROG_INFO(V_PRDN2_VEHL_CD,
	    				   					    SAM_BN_SN,
	    				   						P_EXPD_CO_CD,
	    				   						SAM_APL_YMD,
                                                SAM_VIN,
...
-- 프로시저 hkomms.LOAD_PROD_MST_ERP_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_PROD_MST_ERP_KMC`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_APL_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4),
                                        IN P_ET_GUBN_CD VARCHAR(2))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_PROD_MST_ERP_KMC
 * Procedure 설명 : ERP로 부터 생산실적 로드
 *                 내수인 경우에는 국가코드를 A99VA로 입력하도록 한다.
 *                 용도차가 아닌 경우에만 작업을 진행한다.
 *                 00공정의 경우 현공정위치의 날짜가 변경일시가 된다.
 *                 PPMS의 연식을 우선은 그냥 그대로 사용한다.
 *                 연식이 지정되지 않은 경우에는 임시로 PPMS 연식을 이용하여 정보를 조회한다.(상용 차종을 위한 조치사항)
 *                 투입일자를 현재공정위치일자와 동일하게 처리해 준다.	
 *                 블랙 리스트 관련 조회 후 처리
 *                 현재 공정이 투입공정 이전의 공정이라면 아래의 작업을 수행한다.
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_PROD_MST_ERP_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_APL_YMD:',IFNULL(P_APL_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
	...
	END IF;
			   ...
			   SET LOOP_CNT = LOOP_CNT + 1;	 
			   CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'F', CONCAT(LOOP_CNT , ':LOAD_PROD_MST_ERP_KMC > CURR_YMD:' , P_CURR_YMD , ':APL_YMD:' , P_APL_YMD , ':ET_GUBN_CD:' , P_ET_GUBN_CD , ':VIN:' , V_VIN_1 ));	 
			  /***************	 
		       * DATA PARSING *	 
		       ***************/	 
			   SET SAM_PRDN_MST_VEHL_CD   = V_PRDN_MST_VEHL_CD_1;	 
			   ...
			   IF SAM_POW_LOC_CD <> '00' AND CHECK_VALID_DATA(SAM_PRDN_ORD_NO, SAM_DEST_NAT_CD) = 'Y' THEN	 
				   ...
				   /*CALL SP_GET_MANUAL_INFO_KMC(SAM_APL_YMD,	 
				   				   P_EXPD_CO_CD,	 
	   			 				   SAM_PRDN_MST_VEHL_CD,	 
	   			 				   SAM_MO_PACK_CD,	 
								   SAM_DEST_NAT_CD,	 
				...
				ELSE
					...
					INTO V_QLTY_VEHL_CD	 
					FROM TB_ALTN_VEHL_MGMT	 
					WHERE PRDN_VEHL_CD = SAM_PRDN_MST_VEHL_CD	 
					AND PRVS_SCN_CD = 'A';
				END IF;	 
				...
				IF V_QLTY_VEHL_CD IS NOT NULL AND V_EXPD_NAT_CD IS NOT NULL THEN
					...
					/*특수한 경우의 연식변경 작업 수행
					  별도 프로시져에서 자체 적용함	
					CALL SP_GET_MDL_MDY_CD_EXTRA_KMC(SAM_APL_YMD,	 
						   						P_EXPD_CO_CD,	 
						   						V_QLTY_VEHL_CD,	 
												V_EXPD_NAT_CD,	 
												SAM_MO_PACK_CD,	 
               ...
               IF V_WIOUT_DESNAT = 'Y' THEN   /* 제외 대리점 코드가 아니면 실행	  */
				   /*현재 공정이 투입공정 이전의 공정이라면 아래의 작업을 수행한다.	  */
				   IF V_TRWI_LOC_CD IS NULL THEN
					  CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'F', CONCAT(LOOP_CNT , ':LOAD_PROD_MST_ERP_KMC > 투입공정 이전 데이터:' , SAM_VIN , ':QLTY_VEHL_CD:' , V_QLTY_VEHL_CD , ':PRDN_VEHL_CD:' , V_PRDN2_VEHL_CD , ':MDL_MDY_CD:' , V_MDL_MDY_CD , ':EXPD_NAT_CD:' , V_EXPD_NAT_CD , ':V_TMP_TRWI_YMD:' , V_TMP_TRWI_YMD , ':V_TRWI_LOC_CD:' , V_TRWI_LOC_CD , ':'));	 
					  /*특수한 경우의 연식변경 작업 다시 한번 수행 (APL_YMD가 바뀔 경우 연식이 달라질 수 있음)
					     별도 프로시져에서 자체 적용함
			   		  CALL SP_GET_MDL_MDY_CD_EXTRA_KMC(SAM_APL_YMD,	 
			   										        P_EXPD_CO_CD,	 
				   ...
				   /*현재 공정이 투입공정 이후의 공정이라면 아래의 작업을 수행한다.	  */
				   ELSE
					   CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ':LOAD_PROD_MST_ERP_KMC > 투입공정 데이터:' , SAM_VIN , ':QLTY_VEHL_CD:' , V_QLTY_VEHL_CD , ':PRDN_VEHL_CD:' , V_PRDN2_VEHL_CD , ':MDL_MDY_CD:' , V_MDL_MDY_CD , ':EXPD_NAT_CD:' , V_EXPD_NAT_CD , ':V_TMP_TRWI_YMD:' , V_TMP_TRWI_YMD , ':V_TRWI_LOC_CD:' , V_TRWI_LOC_CD , ':'));
	 
					   CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : LOAD_PROD_MST_ERP_KMC > 투입공정 데이터[' , SAM_VIN , '] : QLTY_VEHL_CD(' , V_QLTY_VEHL_CD , '), PRDN_VEHL_CD(' , V_PRDN2_VEHL_CD , '), MDL_MDY_CD(' , V_MDL_MDY_CD , '), EXPD_NAT_CD(' , V_EXPD_NAT_CD , '), V_TMP_TRWI_YMD(' , V_TMP_TRWI_YMD , '), V_PREV_TRWI_YMD(' , V_PREV_TRWI_YMD , '), P_APL_YMD(' , P_APL_YMD , '), DATE_DIFF_CNT(' , '7' , ')'));	 
	 

					   IF V_PREV_TRWI_YMD IS NULL THEN	 
						 CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : LOAD_PROD_MST_ERP_KMC > V_PREV_TRWI_YMD IS NULL'));	

                         IF V_TMP_TRWI_YMD < DATE_FORMAT(DATE_SUB(STR_TO_DATE(P_APL_YMD,'%Y%m%d'), INTERVAL V_DATE_DIFF_CNT DAY), '%Y%m%d')  THEN	 
                            CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : IF LOAD_PROD_MST_ERP_KMC > V_TMP_TRWI_YMD < DATE_FORMAT(STR_TO_DATE(P_APL_YMD) - DATE_DIFF_CNT'));	 
							...
							CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : LOAD_PROD_MST_ERP_KMC > 투입공정 데이터[' , SAM_VIN , '] : 투입일(' , V_TMP_TRWI_YMD , ')이 Summary 작업 시작일자(' ,  DATE_FORMAT(DATE_SUB(STR_TO_DATE(P_APL_YMD,'%Y%m%d'), INTERVAL V_DATE_DIFF_CNT DAY), '%Y%m%d') , ') 이전 이어서 제외.'));	 
	 
                         ELSE	 
                            CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : ELSE LOAD_PROD_MST_ERP_KMC > V_TMP_TRWI_YMD < DATE_FORMAT(STR_TO_DATE(P_APL_YMD) - DATE_DIFF_CNT'));	 
	 
                            /*Summary 작업 시작일 이내이고 아직 한번도 투입되지 않은 경우에는 적용일과 투입일을 모두 실제로 투입된 일자로	 
							  변경하여 준다.	  */
                            SET V_TRWI_USED_YN = 'N';
							SET V_TRWI_YMD  = V_TMP_TRWI_YMD;	 
							SET SAM_APL_YMD = V_TMP_TRWI_YMD;	 
	 
							CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : LOAD_PROD_MST_ERP_KMC > 투입공정 데이터[' , SAM_VIN , '] : 투입일(' , V_TMP_TRWI_YMD , ')로 투입 처리.'));
                         END IF;
					   ELSE	 
						  /*당일 투입처리된 항목이 있다면 그것을 덮어 쓰게 되므로 아래와 같이 V_TRWI_USED_YN 값을 'N' 으로 해 주어야 한다. */
						  IF V_PREV_TRWI_YMD = V_TRWI_YMD THEN
							 SET V_TRWI_USED_YN = 'N';
							 CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT(LOOP_CNT , ' : LOAD_PROD_MST_ERP_KMC > 투입공정 데이터[' , SAM_VIN , '] : 현재공정일자(' , V_TRWI_YMD , ')와 이전 공정일자가 같아 덮어씀.'));
						  ELSEIF V_TRWI_YMD < V_PREV_TRWI_YMD THEN
							   UPDATE TB_PROD_MST_INFO	 
						 	   SET TRWI_USED_YN = 'Y',	 
						     	   MDFY_DTM = SYSDATE(),	 
                   ...
                   CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT('BEFORE SP_UPDATE_PROD_MST_PROG_INFO : [' , LOOP_CNT , ']'));	 
				   /*현재 진행중인 물량 정보 업데이트 작업 수행	  */
				   CALL SP_UPDATE_PROD_MST_PROG_INFO(V_PRDN2_VEHL_CD,	 
	    				   					    SAM_BN_SN,	 
	    				   						P_EXPD_CO_CD,	 
	...
	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'S', CONCAT('LOAD_PROD_MST_ERP_KMC LOOP COUNT : [' , LOOP_CNT , ']'));
...
-- 프로시저 hkomms.LOAD_PROD_MST_INFO_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_PROD_MST_INFO_HMC`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4),
                                        IN P_ET_GUBN_CD VARCHAR(2))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_PROD_MST_INFO_HMC
 * Procedure 설명 : 작업전 I/F 테이블에서 작업내역 I/F 처리로 표시
 *                 생산마스터배치작업
 * 입력 파라미터    :  P_CURR_YMD                   현재년월일
 *                 P_EXPD_CO_CD                 회사코드
 *                 P_ET_GUBN_CD                 전송구분코드
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_PROD_MST_INFO_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
							,',V_APL_YMD:',IFNULL(V_APL_YMD,'')
							,',V_DATE_DIFF_CNT:',IFNULL(CONCAT(V_DATE_DIFF_CNT),''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
			END IF;
    ...
    SET CURR_LOC_NUM = 1;
		SET V_DATE_DIFF_CNT=7;
			...
			IF P_ET_GUBN_CD = '01' THEN
				...
				SET V_APL_YMD = P_CURR_YMD;
			ELSE
				/* 오전에 들어오는 데이터는 적용일자가 어제 기준이다. */
				
				SET V_APL_YMD = DATE_FORMAT(DATE_SUB(STR_TO_DATE(P_CURR_YMD, '%Y%m%d'), INTERVAL 1 DAY), '%Y%m%d');
			END IF;	   	
    ...
    SET CURR_LOC_NUM = 2;
			...
			/******************************************두개 파라미터 내용을 채워서 넘기도록 함 *************************/			    
			CALL LOAD_PROD_MST_ERP_HMC(P_CURR_YMD, 
			                           V_APL_YMD, 
									   P_EXPD_CO_CD, 
									   P_ET_GUBN_CD
									   );
...
-- 프로시저 hkomms.LOAD_PROD_MST_INFO_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `LOAD_PROD_MST_INFO_KMC`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_EXPD_CO_CD VARCHAR(4),
                                        IN P_ET_GUBN_CD VARCHAR(2))
BEGIN
/***************************************************************************
 * Procedure 명칭 : LOAD_PROD_MST_INFO_KMC
 * Procedure 설명 : ERP 전환	
 *                 작업전 I/F 테이블에서 작업내역 I/F 처리로 표시
 *                 삭제 대신에 업데이트로 변경
 *                 2회 전송으로 적용 일자 변경
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('LOAD_PROD_MST_INFO_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
							,',V_APL_YMD:',IFNULL(V_APL_YMD,'')
    ...
    SET CURR_LOC_NUM = 1;
		SET V_DATE_DIFF_CNT=7;
			...
			IF P_EXPD_CO_CD = '02' THEN	 
               /*ERP 전환	 
                 작업전 I/F 테이블에서 작업내역 I/F 처리로 표시	 */ 
			   UPDATE TB_PROD_MST_INFO_ERP_KMC	 
			      SET RCPM_TRTM_YN = 'N'
			    WHERE APL_YMD = V_APL_YMD;
    ...
    SET CURR_LOC_NUM = 2;
				...
				/******************************************두개 파라미터 내용을 채워서 넘기도록 함 *************************/	 
			   CAll LOAD_PROD_MST_ERP_KMC(P_CURR_YMD, 
			                              V_APL_YMD, 
										  P_EXPD_CO_CD, 
										  P_ET_GUBN_CD
										  );
	 
			   /*2010-08-14.김동근 삭제 대신에 업데이트로 변경	 
			     2016-06-01.1일 2회 전송으로 적용 일자 변경	  */
...
-- 프로시저 hkomms.OWNERS_DATE_CHANGE_BATCH 구조 내보내기
DELIMITER //
CREATE PROCEDURE `OWNERS_DATE_CHANGE_BATCH`()
BEGIN
/***************************************************************************
 * Procedure 명칭 : OWNERS_DATE_CHANGE_BATCH
 * Procedure 설명 : 일자변경시에 작업 수행(배치 실행)
 *                 반드시 현재일 시작시간에 돌려야 한다.
 *                 아래의 작업들은 오라클 데드락 방지를 위하여 하나의 세션안에서 실행되어야 한다.
 *                 PDI 재고 일자 변경 배치 수행	 
 *                 세화 재고 일자 변경 배치 수행	 
 *                 생산계획 일자변경 배치 수행	
 *                 생산마스터 일자변경 배치 수행	
 ...
 * 최초작성          2023-04-10     안상천   최초 전환함
 ****************************************************************************/
	DECLARE V_EXPD_CO_CD      VARCHAR(2);
	DECLARE STRT_DATE  DATETIME;
	DECLARE CURR_YMD   VARCHAR(8);
	DECLARE SRCH_YMD 	VARCHAR(8);
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('OWNERS_DATE_CHANGE_BATCH',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_EXPD_CO_CD:',IFNULL(V_EXPD_CO_CD,'')
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
    ...
    SET CURR_LOC_NUM = 1;
			...
			/*[중요] 아래의 작업들은 오라클 데드락 방지를 위하여 하나의 세션안에서 실행되어야 한다.	 	 
              PDI 재고 일자 변경 배치 수행	  */
			CALL SP_PDI_IV_INFO_BATCH();	 
	 

    SET CURR_LOC_NUM = 2;

            /*세화 재고 일자 변경 배치 수행	  */
			CALL SP_SEWON_IV_INFO_BATCH();
	 

    SET CURR_LOC_NUM = 3;
	 
			/*HMC KMC 생산계획 일자변경 배치 수행	  */
			/*SET V_EXPD_CO_CD = '01';
			CALL SP_APS_INTERFACE_DATE_HMC(V_EXPD_CO_CD);
			SET V_EXPD_CO_CD = '02';
			CALL SP_APS_INTERFACE_DATE_KMC(V_EXPD_CO_CD);*/
			/* start */
		 	SET STRT_DATE  = SYSDATE();
    ...
    SET CURR_LOC_NUM = 7;
			...
			/*HMC KMC 생산마스터 일자변경 배치 수행	  */
			/*SET V_EXPD_CO_CD = '01';
			CALL PROD_MST_INTERFACE_DATE_HMC(V_EXPD_CO_CD);		
			SET V_EXPD_CO_CD = '02';
			CALL PROD_MST_INTERFACE_DATE_KMC(V_EXPD_CO_CD);*/
    ...
    SET CURR_LOC_NUM = 8;
		 	...
		 	IF SRCH_YMD IS NULL THEN		 	
				SET SRCH_YMD = CURR_YMD;
		 	END IF;
		 
    SET CURR_LOC_NUM = 9;
		 	CALL SP_GET_PROD_MST_SUM_DTL_HMC(CURR_YMD, SRCH_YMD, V_EXPD_CO_CD);	 
			/* end */
    ...
    SET CURR_LOC_NUM = 12;
		 	CALL SP_GET_PROD_MST_SUM_DTL_KMC(CURR_YMD, SRCH_YMD, V_EXPD_CO_CD);		 
		 
    SET CURR_LOC_NUM = 13;
		 	CALL WRITE_BATCH_LOG('0.4 생산마스터일자변경배치작업_HMC', STRT_DATE, 'S', '배치처리완료');
			/* end */
...
-- 이벤트 hkomms.OWNERS_DATE_CHANGE_BATCH_0600 구조 내보내기
DELIMITER //
CREATE EVENT `OWNERS_DATE_CHANGE_BATCH_0600` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 06:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL OWNERS_DATE_CHANGE_BATCH()//
DELIMITER ;

-- 프로시저 hkomms.OWNERS_DATE_CHANGE_BATCH_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `OWNERS_DATE_CHANGE_BATCH_HMC`(IN P_BTCH_USER_EENO VARCHAR(20),IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : OWNERS_DATE_CHANGE_BATCH_HMC
 * Procedure 설명 : 일자변경시에 작업 수행(배치 실행)
 *                 반드시 현재일 시작시간에 돌려야 한다.
 *                 PDI 재고 일자 변경 배치 수행 
 *                 세화 재고 일자 변경 배치 수행 
 *                 생산계획 일자변경 배치 수행
 *                 생산마스터 일자변경 배치 수행
 *                 위의 작업들이 모두 이루어진 뒤에 아래의 작업이 진행되어야 한다.
 *                 (일자 변경 배치 수행시에 세원 재고 배치가 이루어진 뒤에 반드시 PDI 재고 배치가 이루어 져야 한다.
 *                 PDI 재고 재계산 로직 내부에 세원재고 재계산 로직이 포함되어 있기 때문이다.
 *                 리고 PDI 재고 에 없는 항목이 세원재고에 있을 수도 있으므로 세원 일자 변경 재치 수행시에는 세원재고 재계산 작업은 또한 진행되어야 한다.)
 * 입력 파라미터    :  P_EXPD_CO_CD                 회사코드
 * 리턴값         :  해당사항없음
 ****************************************************************************
 * 작업내용
 ...
 ****************************************************************************/
	     ...
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE) 
			           VALUES (
			          		CONCAT('OWNERS_DATE_CHANGE_BATCH_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
    ...
    SET CURR_LOC_NUM = 1;
			...
			SET P_BTCH_USER_EENO = V_BATCH_USER_EENO;


			/*[중요] 아래의 작업들은 오라클 데드락 방지를 위하여 하나의 세션안에서 실행되어야 한다.
              PDI 재고 일자 변경 배치 수행  */
			CALL SP_PDI_IV_INFO_BATCH(P_EXPD_CO_CD);
            

    SET CURR_LOC_NUM = 2;

            /*세화 재고 일자 변경 배치 수행  */
			CALL SP_SEWON_IV_INFO_BATCH(P_EXPD_CO_CD);
            

    SET CURR_LOC_NUM = 3;

			/*HMC 생산계획 일자변경 배치 수행  */
			CALL SP_APS_INTERFACE_DATE_HMC(P_EXPD_CO_CD);


    SET CURR_LOC_NUM = 4;

			/*HMC 생산마스터 일자변경 배치 수행  */
			CALL PROD_MST_INTERFACE_DATE_HMC(P_EXPD_CO_CD);
...
-- 프로시저 hkomms.OWNERS_DATE_CHANGE_BATCH_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `OWNERS_DATE_CHANGE_BATCH_KMC`(IN P_BTCH_USER_EENO VARCHAR(20),IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : OWNERS_DATE_CHANGE_BATCH_KMC
 * Procedure 설명 : 일자변경시에 작업 수행(배치 실행)
 *                 반드시 현재일 시작시간에 돌려야 한다.
 *                 아래의 작업들은 오라클 데드락 방지를 위하여 하나의 세션안에서 실행되어야 한다.
 *                 PDI 재고 일자 변경 배치 수행	 
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE) 
			           VALUES (
			          		CONCAT('OWNERS_DATE_CHANGE_BATCH_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_BTCH_USER_EENO:',IFNULL(P_BTCH_USER_EENO,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,''))
							,SYSDATE()
			          	);
    ...
    SET CURR_LOC_NUM = 3;
			...
			/*KMC 생산계획 일자변경 배치 수행	  */
			CALL SP_APS_INTERFACE_DATE_KMC(P_EXPD_CO_CD);
	 

    SET CURR_LOC_NUM = 4;

			/*KMC 생산마스터 일자변경 배치 수행	  */
			CALL PROD_MST_INTERFACE_DATE_KMC(P_EXPD_CO_CD);
...
-- 프로시저 hkomms.PROD_MST_INTERFACE_DATE_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `PROD_MST_INTERFACE_DATE_HMC`(IN P_EXPD_CO_CD VARCHAR(4))
BEGIN 
/***************************************************************************
 * Procedure 명칭 : PROD_MST_INTERFACE_DATE_HMC
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('PROD_MST_INTERFACE_DATE_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',SRCH_YMD:',IFNULL(SRCH_YMD,'')
							,',STRT_DATE:',IFNULL(DATE_FORMAT(STRT_DATE, '%Y%m%d'),''))
				...
				CALL WRITE_BATCH_LOG('0.ERR 생산마스터일자변경배치작업_HMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
    ...
    SET CURR_LOC_NUM = 2;
		 ...
		 CALL SP_GET_PROD_MST_SUM_DTL_HMC(CURR_YMD, SRCH_YMD, P_EXPD_CO_CD);
...
-- 프로시저 hkomms.PROD_MST_INTERFACE_DATE_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `PROD_MST_INTERFACE_DATE_KMC`(IN P_EXPD_CO_CD VARCHAR(4))
BEGIN
/***************************************************************************
 * Procedure 명칭 : PROD_MST_INTERFACE_DATE_KMC
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('PROD_MST_INTERFACE_DATE_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',SRCH_YMD:',IFNULL(SRCH_YMD,'')
							,',STRT_DATE:',IFNULL(DATE_FORMAT(STRT_DATE, '%Y%m%d'),''))
				...
				CALL WRITE_BATCH_LOG('생산마스터일자변경배치작업_KMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
    ...
    SET CURR_LOC_NUM = 2;
		 ...
		 CALL SP_GET_PROD_MST_SUM_DTL_KMC(CURR_YMD, SRCH_YMD, P_EXPD_CO_CD);	 
	 

    SET CURR_LOC_NUM = 3;
 
	 
		 CALL WRITE_BATCH_LOG('생산마스터일자변경배치작업_KMC', STRT_DATE, 'S', '배치처리완료');	

    SET CURR_LOC_NUM = 4;
END//
DELIMITER ;

-- 프로시저 hkomms.PROD_MST_INTERFACE_HMC2 구조 내보내기
DELIMITER //
CREATE PROCEDURE `PROD_MST_INTERFACE_HMC2`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_ET_GUBN_CD VARCHAR(2),
                                        IN P_EXPD_CO_CD VARCHAR(4))
PROC_BODY : BEGIN
/***************************************************************************
 * Procedure 명칭 : PROD_MST_INTERFACE_HMC2
 * Procedure 설명 : 생산마스터 인터페이스 처리
 * 입력 파라미터    :  P_CURR_YMD                   현재년월일
 *                 P_ET_GUBN_CD                 전송구분코드
 *                 P_EXPD_CO_CD                 회사코드
 * 리턴값         :  해당사항없음
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('PROD_MST_INTERFACE_HMC2',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							,',PREV_YMD:',IFNULL(PREV_YMD,'')
							,',V_TO_YMD:',IFNULL(V_TO_YMD,'')
							,',V_PRD_STAT:',IFNULL(V_PRD_STAT,'')
							,',V_FNH_STAT:',IFNULL(V_FNH_STAT,'')
							,',V_APL_YMD:',IFNULL(V_APL_YMD,'')
							,',V_CURR_YMD:',IFNULL(V_CURR_YMD,'')
							,',V_ET_GUBN_CD:',IFNULL(V_ET_GUBN_CD,'')
							,',STRT_DATE:',IFNULL(DATE_FORMAT(STRT_DATE, '%Y%m%d'),'')
							,',V_CHK_HMC:',IFNULL(CONCAT(V_CHK_HMC),''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
			          
				CALL WRITE_BATCH_LOG('2.ERR 생산마스터배치작업_HMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
	...
	/* [주의] 이전의 데이터를 배치작업 진행하여야 할 경우에는 CURR_YMD, 기간(P_DATE_DIFF_CNT) 값을 적절하게 변경하여 준다.  */
	SET CURR_YMD = P_CURR_YMD;

	SET PREV_YMD = DATE_FORMAT(DATE_SUB(STR_TO_DATE(CURR_YMD, '%Y%m%d'), INTERVAL V_DATE_DIFF_CNT DAY), '%Y%m%d');

	IF P_ET_GUBN_CD = '01' THEN
		/*  12시 이후 오후에 들어오는 데이터는 적용일자가 오늘 */
		SET V_APL_YMD = CURR_YMD;
	ELSE
		/*  오전에 들어오는 데이터는 적용일자가 어제 기준이다. */
		SET V_APL_YMD = DATE_FORMAT(DATE_SUB(STR_TO_DATE(CURR_YMD, '%Y%m%d'), INTERVAL 1 DAY), '%Y%m%d');
	END IF;

	SET V_PRD_STAT = GET_ERP_ET_INFO_EXIST_YN(V_APL_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD); 
		
	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', CONCAT('PG_INTERFACE_ERP_ET_INFO.GET_ERP_ET_INFO_EXIST_YN : ' ,  V_PRD_STAT));


    SET CURR_LOC_NUM = 2;

	SET V_FNH_STAT = GET_BTCH_FNH_YN(CURR_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD);

	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', CONCAT('GET_BTCH_FNH_YN : ' ,  V_FNH_STAT));
	...
	CALL WRITE_BATCH_LOG('2.1 생산마스터배치작업_HMC', STRT_DATE, 'S', CONCAT('ERP내역존재:' ,  V_PRD_STAT ,  ', 기처리여부:' , V_FNH_STAT));
	...
	CALL LOAD_PROD_MST_INFO_HMC(CURR_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD ); /*  ERP 데이터 가져옴  */
		 

    SET CURR_LOC_NUM = 6;

	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', 'LOAD_PROD_MST_INFO_HMC');
		 		 		 

    SET CURR_LOC_NUM = 7;

	CALL SP_GET_PROD_MST_SUM2_HMC(PREV_YMD, CURR_YMD, V_APL_YMD, P_EXPD_CO_CD);  /*  재고보정 함 */
	

    SET CURR_LOC_NUM = 8;

	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', '');


    SET CURR_LOC_NUM = 9;

	/* 배치완료 정보 저장  */
	CALL SAVE_PROD_MST_BTCH_FNH_INFO(CURR_YMD, P_ET_GUBN_CD, P_EXPD_CO_CD);


    SET CURR_LOC_NUM = 10;

	CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_HMC', SYSDATE(), 'S', 'SAVE_PROD_MST_BTCH_FNH_INFO');
	...
	/* 미지정 국가 항목 메일 전송  */
	CALL SEND_NOAPIM_NATL_INFO_MAIL(P_EXPD_CO_CD, '03', CURR_YMD);
		 

    SET CURR_LOC_NUM = 12;

	CALL WRITE_BATCH_LOG(CONCAT('2.2 생산마스터배치작업_HMC_' ,  P_ET_GUBN_CD), STRT_DATE, 'S', '배치처리완료');
...
-- 프로시저 hkomms.PROD_MST_INTERFACE_KMC2 구조 내보내기
DELIMITER //
CREATE PROCEDURE `PROD_MST_INTERFACE_KMC2`(IN P_CURR_YMD VARCHAR(8),
                                        IN P_ET_GUBN_CD VARCHAR(2),
                                        IN P_EXPD_CO_CD VARCHAR(4))
PROC_BODY : BEGIN
/***************************************************************************
 * Procedure 명칭 : PROD_MST_INTERFACE_KMC2
 * Procedure 설명 : 생산마스터 인터페이스 처리
 * 입력 파라미터    :  P_CURR_YMD                   현재년월일
 *                 P_ET_GUBN_CD                 전송구분코드
 *                 P_EXPD_CO_CD                 회사코드
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('PROD_MST_INTERFACE_KMC2',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',P_CURR_YMD:',IFNULL(P_CURR_YMD,'')
							,',P_ET_GUBN_CD:',IFNULL(P_ET_GUBN_CD,'')
							,',P_EXPD_CO_CD:',IFNULL(P_EXPD_CO_CD,'')
							,',CURR_YMD:',IFNULL(CURR_YMD,'')
							...
							,',V_ET_GUBN_CD:',IFNULL(V_ET_GUBN_CD,'')
							,',STRT_DATE:',IFNULL(DATE_FORMAT(STRT_DATE, '%Y%m%d'),'')
							,',V_CHK_KMC:',IFNULL(CONCAT(V_CHK_KMC),''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
			          
				CALL WRITE_BATCH_LOG('생산마스터배치작업_KMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
	...
	/*  ERP 데이터 전송완료 시간이 들어왔는지 체크한다.	  */
	SET V_PRD_STAT = GET_ERP_ET_INFO_EXIST_YN(V_APL_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD);	 
	/*  배치 수행여부 체크는 APL_YMD가 아닌 CURR_YMD 기준으로 체크한다.	  */
	/*  '03' - 오전 배치, '04' - 오후 배치	  */
	SET V_FNH_STAT = GET_BTCH_FNH_YN(CURR_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD);	 
	...
	/* 2012.12.26. ERP 데이터 만을 이용하여 재고 보정하도록 로직 변경	  */
	/*  ERP 자료 조회 후 없으면 JOB NEXT_DATE 10 분 후로 수정	  */
	SELECT COUNT(*) AS CNT	 
		INTO V_CHK_KMC	 
	FROM TB_PROD_MST_INFO_ERP_KMC	 
	WHERE APL_YMD = V_APL_YMD	 
	...
	CALL LOAD_PROD_MST_INFO_KMC(CURR_YMD, P_EXPD_CO_CD, P_ET_GUBN_CD); /*   ERP 데이터 가져옴	  */
	 

    SET CURR_LOC_NUM = 4;

	CALL SP_GET_PROD_MST_SUM2_KMC(PREV_YMD, CURR_YMD, V_APL_YMD, P_EXPD_CO_CD);  /*  재고보정	  */
	...
	IF P_ET_GUBN_CD = '01' THEN	 
		CALL WRITE_BATCH_LOG('생산마스터배치작업_KMC_01', STRT_DATE, 'S', '배치처리완료');	 
	ELSE	 
		CALL WRITE_BATCH_LOG('생산마스터배치작업_KMC_02', STRT_DATE, 'S', '배치처리완료');	 
	END IF;	 
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_LOG('미지정 국가 메일 발송', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
...
DELIMITER //
CREATE PROCEDURE `SET_ERP_ET_INFO`()
BEGIN 
/***************************************************************************
 ...
 ****************************************************************************/
    ...
    CALL SET_ERP_ET_INFO_HMC();
   
    SET CURR_LOC_NUM = 2;
   
    CALL SET_ERP_ET_INFO_KMC();
...
-- 프로시저 hkomms.SET_ERP_ET_INFO_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SET_ERP_ET_INFO_HMC`()
BEGIN
/***************************************************************************
 * Procedure 명칭 : SET_ERP_ET_INFO_HMC
 * Procedure 설명 : 생산마스터 데이터 전송 완료 일자 설정
 * 입력 파라미터    :  P_DL_EXPD_CO_CD   취급설명서회사코드
 * 리턴값         :  해당사항없음
 ****************************************************************************
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SET_ERP_ET_INFO_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_CURR_YMD:',IFNULL(V_CURR_YMD,'')
							,',V_APL_YMD:',IFNULL(V_APL_YMD,'')
							,',V_DL_EXPD_CO_CD:',IFNULL(V_DL_EXPD_CO_CD,'')
							,',V_ET_GUBN_CD:',IFNULL(V_ET_GUBN_CD,'')
							,',V_CNT_KMC:',IFNULL(CONCAT(V_CNT_HMC),''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
			END IF;
    ...
    SET CURR_LOC_NUM = 1;
			SET V_DL_EXPD_CO_CD = '01';

			SET V_CURR_YMD = DATE_FORMAT(SYSDATE(), '%Y%m%d');
			SET V_APL_YMD  = DATE_FORMAT(DATE_SUB(SYSDATE(), INTERVAL 12 HOUR), '%Y%m%d');
    ...
    SET CURR_LOC_NUM = 3;
            IF V_CNT_HMC > 0 THEN
	            UPDATE TB_BATCH_ERP_ET_INFO
	             ...
	             WHERE DL_EXPD_CO_CD = V_DL_EXPD_CO_CD
	            ...
	            IF V_EXCNT = 0 THEN
	                ...
	                (
	                 ...
	                 VALUES
	                (
	                 V_DL_EXPD_CO_CD, V_APL_YMD, V_ET_GUBN_CD, SYSDATE()
	                 );
	            END IF;
            END IF;       
    SET CURR_LOC_NUM = 4;     
...
-- 이벤트 hkomms.SET_ERP_ET_INFO_HMC_0601 구조 내보내기
DELIMITER //
CREATE EVENT `SET_ERP_ET_INFO_HMC_0601` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 06:01:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SET_ERP_ET_INFO_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SET_ERP_ET_INFO_HMC_1455 구조 내보내기
DELIMITER //
CREATE EVENT `SET_ERP_ET_INFO_HMC_1455` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 14:55:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SET_ERP_ET_INFO_HMC()//
DELIMITER ;

-- 프로시저 hkomms.SET_ERP_ET_INFO_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SET_ERP_ET_INFO_KMC`()
BEGIN
/***************************************************************************
 * Procedure 명칭 : SET_ERP_ET_INFO_KMC
 * Procedure 설명 : 생산마스터 데이터 전송 완료 일자 설정
 * 입력 파라미터    :  P_DL_EXPD_CO_CD   취급설명서회사코드
 * 리턴값         :  해당사항없음
 ****************************************************************************
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SET_ERP_ET_INFO_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_CURR_YMD:',IFNULL(V_CURR_YMD,'')
							,',V_APL_YMD:',IFNULL(V_APL_YMD,'')
							,',V_DL_EXPD_CO_CD:',IFNULL(V_DL_EXPD_CO_CD,'')
							,',V_ET_GUBN_CD:',IFNULL(V_ET_GUBN_CD,'')
...
-- 이벤트 hkomms.SET_ERP_ET_INFO_KMC_0525 구조 내보내기
DELIMITER //
CREATE EVENT `SET_ERP_ET_INFO_KMC_0525` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 05:25:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SET_ERP_ET_INFO_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SET_ERP_ET_INFO_KMC_1405 구조 내보내기
DELIMITER //
CREATE EVENT `SET_ERP_ET_INFO_KMC_1405` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 14:05:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SET_ERP_ET_INFO_KMC()//
DELIMITER ;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_LOG('1.ERR APS배치작업_HMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_LOG('APS배치작업_KMC', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_LOG('날짜정보배치', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
 ...
 ****************************************************************************/
	...
	END IF;					
				...
				CALL WRITE_BATCH_EXE_LOG('SP_GET_PROD_MST_SUM_DTL_KMC', SYSDATE(), 'S', CONCAT('DATA_SN : [' , V_DATA_SN_1 , '], APL_YMD : [' , CURR_YMD , '], QLTY_VEHL_CD : [' , V_QLTY_VEHL_CD_1 , '], MDL_MDY_CD : [' , V_MDL_MDY_CD_1 , '], LANG_CD : [' , V_LANG_CD_1 , ']'));
...
DELIMITER //
CREATE PROCEDURE `SP_GLOVIS_WHSN_INFO_BATCH`()
BEGIN 
/***************************************************************************
 ...
 ****************************************************************************/
    ...
    CALL SP_GLOVIS_WHSN_INFO_BATCH_HMC();
   
    SET CURR_LOC_NUM = 2;
   
    CALL SP_GLOVIS_WHSN_INFO_BATCH_KMC();
...
-- 프로시저 hkomms.SP_GLOVIS_WHSN_INFO_BATCH_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SP_GLOVIS_WHSN_INFO_BATCH_HMC`()
BEGIN 
/***************************************************************************
 * Procedure 명칭 : SP_GLOVIS_WHSN_INFO_BATCH_HMC
 * Procedure 설명 : 글로비스 자동 입고 처리 배치 작업
 * 입력 파라미터    :  
 * 리턴값         :  해당사항없음
 ****************************************************************************
 ...
 ****************************************************************************/
	...
	DECLARE V_EXPD_CO_CD		    VARCHAR(4);
	...
	DECLARE SEWON_WHOT_LIST CURSOR FOR
								   SELECT A.QLTY_VEHL_CD,	 
	   									  ...
	   									  IFNULL(A.DL_EXPD_BOX_QTY, 0) AS DL_EXPD_BOX_QTY,	 
	   									  A.RQ_QTY AS WHSN_QTY,	 
	   									  0 AS DEEI1_QTY,	 
	   									  'SYSTEM' AS USER_EENO,	 
	   									  A.PRDN_PLNT_CD  
								   FROM TB_SEWON_WHOT_INFO	A 
								   WHERE A.DL_EXPD_RQ_SCN_CD = '01'	 
								   AND A.CMPL_YN = 'N'	 
								   AND A.DEL_YN  = 'N'	 
								   AND A.LANG_CD = 'KO'
								   AND  (SELECT COUNT(C.QLTY_VEHL_CD)	 
						                        FROM TB_VEHL_MGMT C	 
									            WHERE C.QLTY_VEHL_CD = A.QLTY_VEHL_CD
									            AND C.DL_EXPD_CO_CD = V_EXPD_CO_CD)>0;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SP_GLOVIS_WHSN_INFO_BATCH_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_EXPD_CO_CD:',IFNULL(V_EXPD_CO_CD,'')
							,',V_QLTY_VEHL_CD_1:',IFNULL(V_QLTY_VEHL_CD_1,'')
...
-- 이벤트 hkomms.SP_GLOVIS_WHSN_INFO_BATCH_HMC_0005 구조 내보내기
DELIMITER //
CREATE EVENT `SP_GLOVIS_WHSN_INFO_BATCH_HMC_0005` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 00:05:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_GLOVIS_WHSN_INFO_BATCH_HMC()//
DELIMITER ;

-- 프로시저 hkomms.SP_GLOVIS_WHSN_INFO_BATCH_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SP_GLOVIS_WHSN_INFO_BATCH_KMC`()
BEGIN 
/***************************************************************************
 * Procedure 명칭 : SP_GLOVIS_WHSN_INFO_BATCH_KMC
 * Procedure 설명 : 글로비스 자동 입고 처리 배치 작업
 * 입력 파라미터    :  
 * 리턴값         :  해당사항없음
 ****************************************************************************
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SP_GLOVIS_WHSN_INFO_BATCH_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_EXPD_CO_CD:',IFNULL(V_EXPD_CO_CD,'')
							,',V_QLTY_VEHL_CD_1:',IFNULL(V_QLTY_VEHL_CD_1,'')
							,',V_DL_EXPD_MDL_MDY_CD_1:',IFNULL(V_DL_EXPD_MDL_MDY_CD_1,'')
							,',V_LANG_CD_1:',IFNULL(V_LANG_CD_1,'')
...
-- 이벤트 hkomms.SP_GLOVIS_WHSN_INFO_BATCH_KMC_0004 구조 내보내기
DELIMITER //
CREATE EVENT `SP_GLOVIS_WHSN_INFO_BATCH_KMC_0004` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 00:04:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_GLOVIS_WHSN_INFO_BATCH_KMC()//
DELIMITER ;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_EXE_LOG('승인취소 에러', SYSDATE(), 'F', CONCAT('SP_PRNT_APVL_INFO_CANCEL 배치처리실패 : [' ,  V_MESSAGE_TEXT ,  ']'));
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_LOG('세원재고배치작업', SYSDATE(), 'F', CONCAT('배치처리실패:[' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_EXE_LOG('승인취소 에러', SYSDATE(), 'F', CONCAT('SP_SEWON_WHSN_INFO_CANCEL 배치처리실패 : [' ,  V_MESSAGE_TEXT ,  ']'));
				COMMIT;
			END IF;
	     END;
...
DELIMITER //
CREATE PROCEDURE `SP_UPDATE_APS_DATA`()
BEGIN 
/***************************************************************************
 ...
 ****************************************************************************/
    ...
    CALL SP_UPDATE_APS_DATA_HMC();
   
    SET CURR_LOC_NUM = 2;
   
    CALL SP_UPDATE_APS_DATA_KMC();
...
-- 프로시저 hkomms.SP_UPDATE_APS_DATA_HMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SP_UPDATE_APS_DATA_HMC`()
BEGIN
/***************************************************************************
 * Procedure 명칭 : SP_UPDATE_APS_DATA_HMC
 * Procedure 설명 : TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행
 * 입력 파라미터    :  P_EXPD_CO_CD                 회사코드
 * 리턴값         :  해당사항없음
 ****************************************************************************
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SP_UPDATE_APS_DATA_HMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_EXPD_CO_CD:',IFNULL(V_EXPD_CO_CD,''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
   
				CALL WRITE_BATCH_EXE_LOG('SP_UPDATE_APS_DATA_HMC', SYSDATE(), 'S', 'SP_UPDATE_APS_DATA_HMC ERROR');
				CALL WRITE_BATCH_LOG('TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행_HMC', SYSDATE(), 'S', '배치처리오류발생_HMC');
			END IF;
	     END;
	...
	CALL SP_APS_INTERFACE_HMC(V_EXPD_CO_CD);

    SET CURR_LOC_NUM = 3;
   
	CALL WRITE_BATCH_EXE_LOG('SP_UPDATE_APS_DATA_HMC', SYSDATE(), 'S', 'SP_UPDATE_APS_DATA_HMC END');
	CALL WRITE_BATCH_LOG('TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행_HMC', SYSDATE(), 'S', '배치처리완료_HMC');
...
-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0700 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0700` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0710 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0710` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:10:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0720 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0720` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:20:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0730 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0730` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:30:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0740 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0740` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:40:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0750 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0750` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:50:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_HMC_0800 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_HMC_0800` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 08:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_HMC()//
DELIMITER ;

-- 프로시저 hkomms.SP_UPDATE_APS_DATA_KMC 구조 내보내기
DELIMITER //
CREATE PROCEDURE `SP_UPDATE_APS_DATA_KMC`()
BEGIN
/***************************************************************************
 * Procedure 명칭 : SP_UPDATE_APS_DATA_KMC
 * Procedure 설명 : TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행
 * 입력 파라미터    :  P_EXPD_CO_CD                 회사코드
 * 리턴값         :  해당사항없음
 ****************************************************************************
 ...
 ****************************************************************************/
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				INSERT INTO TB_DEBUG_INFO (DEBUG_TITL,DEBUG_DATE,WK_STAT,ERR_NO,ERR_SBC)
			           VALUES (
			          		CONCAT('SP_UPDATE_APS_DATA_KMC',':','실행종료위치:',CONCAT(CURR_LOC_NUM),' 오류발생'
							,',V_EXPD_CO_CD:',IFNULL(V_EXPD_CO_CD,''))
							,SYSDATE(),V_RETURNED_SQLSTATE,V_MYSQL_ERRNO,V_MESSAGE_TEXT
			          	);
				COMMIT;
   
				CALL WRITE_BATCH_EXE_LOG('SP_UPDATE_APS_DATA_KMC', SYSDATE(), 'S', 'SP_UPDATE_APS_DATA_KMC ERROR');
				CALL WRITE_BATCH_LOG('TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행_KMC', SYSDATE(), 'S', '배치처리오류발생_KMC');
			END IF;
	     END;
	...
	CALL SP_APS_INTERFACE_KMC(V_EXPD_CO_CD);

    SET CURR_LOC_NUM = 3;
   
	CALL WRITE_BATCH_EXE_LOG('SP_UPDATE_APS_DATA_KMC', SYSDATE(), 'S', 'SP_UPDATE_APS_DATA_KMC END');
	CALL WRITE_BATCH_LOG('TB_BATCH_FNH_INFO 금일정보 삭제후 APS 인터페이스 실행_KMC', SYSDATE(), 'S', '배치처리완료_KMC');
END//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0700 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0700` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0710 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0710` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:10:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0720 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0720` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:20:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0730 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0730` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:30:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0740 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0740` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:40:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0750 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0750` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 07:50:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;

-- 이벤트 hkomms.SP_UPDATE_APS_DATA_KMC_0800 구조 내보내기
DELIMITER //
CREATE EVENT `SP_UPDATE_APS_DATA_KMC_0800` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 08:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_UPDATE_APS_DATA_KMC()//
DELIMITER ;
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	     BEGIN
			...
			IF ERR_CNT > 0 THEN
				...
				CALL WRITE_BATCH_EXE_LOG('생산마스터배치작업_KMC', SYSDATE(), 'F', 
			                   CONCAT('SP_UPDATE_PROD_MST_PROG_INFO : '
			                           	, 'V_PAC_SCN_CD(' , IFNULL(V_PAC_SCN_CD,'') , ')'
                                       	, ', V_PDI_CD(' , IFNULL(V_PDI_CD,'') , ')'
										, ', V_DTL_SN(' , IFNULL(CONCAT(V_DTL_SN),'') , ')'
										, ', TRWI_USED_YN(' , IFNULL(V_TRWI_USED_YN,'') , ')'
										, ', QLTY_VEHL_CD(' , IFNULL(V_QLTY_VEHL_CD,'') , ')'
										, ', MDL_MDY_CD(' , IFNULL(V_MDL_MDY_CD,'') , ')'
										, ', PRDN_MST_VEHL_CD(' , IFNULL(V_PRDN_MST_VEHL_CD,'') , ')'
										, ', BN_SN(' , IFNULL(V_BN_SN,'') , ')'
										, ', APL_YMD(' , IFNULL(V_APL_YMD,'') , ')'
										, ', MO_PACK_CD(' , IFNULL(V_MO_PACK_CD,'') , ')'
										, ', TRWI_YMD(' , IFNULL(V_TRWI_YMD,'') , ')'
										, ', PRDN_PLNT_CD(' , IFNULL(V_PRDN_PLNT_CD,'') , ')'
										, ', VIN(' , IFNULL(V_VIN,'') , ')'
										, ', P_EXPD_CO_CD(' , IFNULL(P_EXPD_CO_CD,'') , ')'
										));
				COMMIT;
...
DELIMITER //
CREATE PROCEDURE `SP_USER_MGMT_BTCH`()
BEGIN
/***************************************************************************
...
-- 이벤트 hkomms.SP_USER_MGMT_BTCH_1930 구조 내보내기
DELIMITER //
CREATE EVENT `SP_USER_MGMT_BTCH_1930` ON SCHEDULE EVERY 1 DAY STARTS '2023-05-15 19:30:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL SP_USER_MGMT_BTCH()//
DELIMITER ;
